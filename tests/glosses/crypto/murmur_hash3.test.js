describe("crypto", "murmurHash3", () => {
    ...
    it("x86_32 does not have collisions when characters share first char code byte", () => {
        ...
        expect(asciiHash).to.not.equal(emojiHash, "Collision detected hashing '⭐' and 'P'!");
    });

    it("should take the inputValidation flag into consideration", () => {
        murmurHash3.inputValidation = false;
        expect(murmurHash3.x86.hash32("invalid input")).to.not.equal(undefined);
        expect(murmurHash3.x86.hash128(["another", "one"])).to.not.equal(undefined);
        expect(murmurHash3.x64.hash128([1234, 5678, 9999])).to.not.equal(undefined);
        expect(murmurHash3.x86.hash32(10010)).to.not.equal(undefined);
        try {
