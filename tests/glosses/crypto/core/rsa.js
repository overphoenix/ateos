const {
    is,
    crypto: { options, md: MD, mgf: MGF, pki: PKI, pss: PSS, random: RANDOM, rsa: RSA, util: UTIL }
} = ateos;
...
describe("rsa", () => {
    // check a pair
    const _pairCheck = function (pair) {
        // PEM check
    ...
    // compare pairs
    const _pairCmp = function (pair1, pair2) {
        const pem1 = {
    ...
    const _genSync = function (options) {
    ...
    const isDeterministic = function (isPrng, isAsync, isPurejs) {
    ...
    it("should generate 512 bit key pair (sync+purejs)", () => {
        // save
        const purejs = options.usePureJavaScript;
        // test pure mode
        options.usePureJavaScript = true;
        _genSync();
        // restore
        options.usePureJavaScript = purejs;
    });
    ...
    it("should generate 512 bit key pair (async+purejs)", (done) => {
        // save
        const purejs = options.usePureJavaScript;
        // test pure mode
        options.usePureJavaScript = true;
    ...
    it("should generate 512 bit key pair (async+workers)", (done) => {
        _genAsync({
            workers: -1
        }, () => {
            done();
    ...
    it("should generate same 512 bit key pair (prng+sync+purejs,prng+sync+purejs)", function () {
    ...
    (function () {
        ...
        algorithms.forEach((algorithm) => {
            it(`should PKCS#8 encrypt and decrypt private key with ${algorithm}`, () => {
                ...
                const encryptedPem = PKI.encryptRsaPrivateKey(
    ...
    (function () {
        ...
        algorithms.forEach((algorithm) => {
            it(`should legacy (OpenSSL style) encrypt and decrypt private key with ${algorithm}`, () => {
                let privateKey = PKI.privateKeyFromPem(_pem.privateKey);
                const encryptedPem = PKI.encryptRsaPrivateKey(
                    privateKey, "password", { algorithm, legacy: true });
                privateKey = PKI.decryptRsaPrivateKey(encryptedPem, "password");
                assert.equal(PKI.privateKeyToPem(privateKey), _pem.privateKey);
            });
        });
    ...
    (function () {
         ...
         *   -pkeyopt rsa_padding_mode:pss -pkeyopt rsa_pss_saltlen:20
         */
        const createTests = function (params) {
            ...
            it(`should rsa encrypt using a ${keySize}-bit key`, () => {
                 ...
                 */
                let key = PKI.publicKeyFromPem(params.publicKeyPem);
