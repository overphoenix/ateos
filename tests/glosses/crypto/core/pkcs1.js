const {
    crypto: { jsbn: JSBN, md: MD, pkcs1: PKCS1, pki: PKI, util: UTIL }
} = ateos;
...
describe("pkcs1", () => {
    ...
    const _bytesToBigInteger = function (bytes) {
        const buffer = UTIL.createBuffer(bytes);
        const hex = buffer.toHex();
        return new BigInteger(hex, 16);
    };

    const _base64ToBn = function (s) {
        const decoded = UTIL.decode64(s);
        return _bytesToBigInteger(decoded);
    };

    const checkOAEPEncryptExamples = function (publicKey, privateKey, md, examples) {
        if (md === "sha1") {
            md = MD.sha1.create();
        } else if (md === "sha256") {
            md = MD.sha256.create();
    ...
    const checkOAEPEncrypt = function (publicKey, privateKey, md, message, seed, expected) {
    ...
    const decodeBase64PublicKey = function (modulus, exponent) {
        modulus = _base64ToBn(modulus);
        exponent = _base64ToBn(exponent);
        return PKI.setRsaPublicKey(modulus, exponent);
    };

    const decodeBase64PrivateKey = function (modulus, exponent, d, p, q, dP, dQ, qInv) {
        modulus = _base64ToBn(modulus);
        exponent = _base64ToBn(exponent);
        d = _base64ToBn(d);
        p = _base64ToBn(p);
    ...
    const makeKey = function () {
        let modulus; let exponent; let d; let p; let q; let dP; let dQ; let qInv; let pubkey; let privateKey;
    ...
    it("should detect invalid RSAES-OAEP padding", () => {
        const keys = makeKey();
