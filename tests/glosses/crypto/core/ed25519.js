const {
    is,
    crypto: { ed25519: ED25519, sha256: SHA256, util: UTIL }
} = ateos;
...
const eb64 = (buffer) => UTIL.encode64(new UTIL.ByteBuffer(buffer).bytes());

const db64 = (x) => new UTIL.ByteBuffer(UTIL.decode64(x), "binary");

const hex = (x) => new UTIL.ByteBuffer(x).toHex();
