const {
    crypto: { pkcs7: PKCS7, pki: PKI, aes: AES, des: DES, util: UTIL }
} = ateos;
...
describe("pkcs7", () => {
    it("should import message from PEM", () => {
        const p7 = PKCS7.messageFromPem(_pem.p7);
    ...
    it("should import indefinite length message from PEM", () => {
        assert.doesNotThrow(() => {
            const p7 = PKCS7.messageFromPem(_pem.p7IndefiniteLength);
            assert.equal(p7.type, PKI.oids.envelopedData);
    ...
    it("should import a message with no certificate", () => {
        const p7 = PKCS7.messageFromPem(_pem.signedDataWithNoCertificate);
        assert.equal(p7.type, PKI.oids.signedData);
    });

    it("should find recipient by serial number", () => {
        const p7 = PKCS7.messageFromPem(_pem.p7);
        const cert = PKI.certificateFromPem(_pem.certificate);

        let ri = p7.findRecipient(cert);
    ...
    it("should 3des-decrypt message", () => {
        const p7 = PKCS7.messageFromPem(_pem.p73des);
        const privateKey = PKI.privateKeyFromPem(_pem.privateKey);
        p7.decrypt(p7.recipients[0], privateKey);
    ...
    it("should aes-encrypt a message", () => {
        const p7 = PKCS7.createEnvelopedData();
        const cert = PKI.certificateFromPem(_pem.certificate);
        const privateKey = PKI.privateKeyFromPem(_pem.privateKey);
        ...
        const ciph = AES.createDecryptionCipher(decryptedKey);
    ...
    it("should 3des-ede-encrypt a message", () => {
        const p7 = PKCS7.createEnvelopedData();
        const cert = PKI.certificateFromPem(_pem.certificate);
        const privateKey = PKI.privateKeyFromPem(_pem.privateKey);
        ...
        // decryption of sym. encrypted data should reveal the content
        const ciph = DES.createDecryptionCipher(decryptedKey);
        ciph.start(p7.encryptedContent.parameter);
        ciph.update(p7.encryptedContent.content);
        ciph.finish();
        assert.equal(ciph.output, "Just a little test");
    ...
    it("should export message to PEM", () => {
        let p7 = PKCS7.createEnvelopedData();
    ...
    it("should decrypt encrypted data from PEM", () => {
        const result = "1f8b08000000000000000b2e494d4bcc5308ce4c4dcfd15130b0b430d4b7343732b03437d05170cc2b4e4a4cced051b034343532d25170492d2d294ecec849cc4b0100bf52f02437000000";
        let key = "b96e4a4c0a3555d31e1b295647cc5cfe74081918cb7f797b";
        key = UTIL.createBuffer(UTIL.hexToBytes(key));

        assert.doesNotThrow(() => {
            const p7 = PKCS7.messageFromPem(_pem.encryptedData);
    ...
    it("should create a degenerate PKCS#7 certificate container", () => {
    ...
    it("should create PKCS#7 SignedData with no attributes", () => {
        // verify with:
    ...
    it("should create PKCS#7 SignedData with content-type, message-digest, and signing-time attributes using GeneralizedTime (1949)", () => {
        // verify with:
        // openssl smime -verify -in p7.pem -signer certificate.pem \
        //   -out signedtext.txt -inform PEM -CAfile certificate.pem
        const p7 = PKCS7.createSignedData();
    ...
    it("should create PKCS#7 SignedData with content-type, message-digest, and signing-time attributes using UTCTime (1950)", () => {
        // verify with:
        // openssl smime -verify -in p7.pem -signer certificate.pem \
        //   -out signedtext.txt -inform PEM -CAfile certificate.pem
        const p7 = PKCS7.createSignedData();
    ...
    it("should create PKCS#7 detached signature", () => {
        const p7 = PKCS7.createSignedData();
        p7.content = UTIL.createBuffer("To be signed.", "utf8");
        p7.addCertificate(_pem.certificate);
        p7.addSigner({
    ...
    it("should create PKCS#7 SignedData with content-type, message-digest, and signing-time attributes using UTCTime (2049)", () => {
        // verify with:
        // openssl smime -verify -in p7.pem -signer certificate.pem \
        //   -out signedtext.txt -inform PEM -CAfile certificate.pem
        const p7 = PKCS7.createSignedData();
    ...
    it("should create PKCS#7 SignedData with content-type, message-digest, and signing-time attributes using GeneralizedTime (2050)", () => {
        // verify with:
        // openssl smime -verify -in p7.pem -signer certificate.pem \
        //   -out signedtext.txt -inform PEM -CAfile certificate.pem
        const p7 = PKCS7.createSignedData();
