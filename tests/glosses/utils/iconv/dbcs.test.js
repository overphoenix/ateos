            ...
            (function (enc) {
                ...
                it(`Decode DBCS encoding '${enc}'`, async () => {
                    ...
                    while (buffer.length > 0) {
                        ...
                        let outp;
                        if (valid) {
                            const outpLength = buffer.readUInt8();
                            outp = buffer.read(outpLength).toBuffer();
                        }
                        const strActual = iconv.decode(inp, enc);

                        if (strActual.charCodeAt(0) >= 0xE000 && strActual.charCodeAt(0) < 0xF900) {
                        ...
                        if (valid) {
                            ...
                            if (strExpected.charCodeAt(0) >= 0xE000 && strExpected.charCodeAt(0) < 0xF900) {
                                // Skip Private use area.
                                continue;
                            }
                ...
                it(`Encode DBCS encoding '${enc}'`, async () => {
                    ...
                    for (let i = 0; i < 0x10000; i++) {
                        ...
                        const bufExpected = buffer.read(len).toBuffer();
                        ...
                        if (strExpected === "3f" && iconvCannotDecodeChars[strActual] === str) {
                            continue;
                        } // Check the iconv cannot encode this char, but we encoded correctly.

                        const str1 = iconv.decode(bufExpected, enc);
                        const str12 = iconv.decode(bufActual, enc);

                        len = buffer.readUInt8();
                        const str2 = buffer.read(len).toString("utf8");
                        len = buffer.readUInt8();
                        const str22 = buffer.read(len).toString("utf8");
                        if (str1 === str && str12 === str && str22 === str &&
                            (str2 === str || (iconvCannotDecodeChars[strActual] === str))) {
                            continue;
                        } // There are multiple ways to encode str, so it doesn't matter which we choose.
