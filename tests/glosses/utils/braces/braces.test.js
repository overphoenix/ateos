...
describe("util", "braces", () => {
    ...
    it("should return an expanded array if defined on options", () => {
        equal("a/{b,c}/d", ["a/b/d", "a/c/d"], { expand: true });
    });

    it("should optimize an array of patterns", () => {
        equal(["a/{b,c}/d", "x/{foo,bar}/z"], ["a/(b|c)/d", "x/(foo|bar)/z"]);
    });

    it("should expand an array of patterns", () => {
        const actual = braces(["a/{b,c}/d", "a/{b,c}/d"]);
        assert.deepEqual(actual, ["a/(b|c)/d", "a/(b|c)/d"]);
    });

    it("should not uniquify by default", () => {
        const actual = braces(["a/{b,c}/d", "a/{b,c}/d"]);
        assert.deepEqual(actual, ["a/(b|c)/d", "a/(b|c)/d"]);
    });

    it("should uniquify when `options.nodupes` is true", () => {
        const actual = braces(["a/{b,c}/d", "a/{b,c}/d"], { nodupes: true });
    ...
    it("should expand ranges that are nested in a set", () => {
        equal("a{b,c,{1..5}}e", ["abe", "ace", "a1e", "a2e", "a3e", "a4e", "a5e"], { expand: true });
    });

    it("should not expand ranges when they are just characters in a set", () => {
    ...
    it("should support optimized nested empty sets", () => {
