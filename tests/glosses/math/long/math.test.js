describe("math", "Long", "math", () => {
    ...
    it("fromBits", () => {
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            assert.equal(val.getLowBits(), TEST_BITS[i + 1]);
        }
    });
    ...
    it("fromNumber", () => {
        ...
        // Test edge cases
        assert.deepEqual(Long.fromNumber(NaN), Long.ZERO);
        assert.deepEqual(Long.fromNumber(Infinity), Long.MAX_VALUE);
    ...
    it("comparisons", () => {
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            for (let j = 0; j < TEST_BITS.length; j += 2) {
                ...
                assert.equal(vi.equals(vj), i === j);
                assert.equal(!vi.equals(vj), i !== j);
                assert.equal(vi.lessThan(vj), i < j);
                assert.equal(vi.lessThanOrEqual(vj), i <= j);
                assert.equal(vi.greaterThan(vj), i > j);
                assert.equal(vi.greaterThanOrEqual(vj), i >= j);
            }
        }
    });
    ...
    it("bit operations", () => {
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            assert.equal(vi.not().getLowBits(), ~TEST_BITS[i + 1]);

            for (let j = 0; j < TEST_BITS.length; j += 2) {
                const vj = Long.fromBits(TEST_BITS[j + 1], TEST_BITS[j]);
                assert.equal(vi.and(vj).getHighBits(), TEST_BITS[i] & TEST_BITS[j]);
                assert.equal(vi.and(vj).getLowBits(), TEST_BITS[i + 1] & TEST_BITS[j + 1]);
                assert.equal(vi.or(vj).getHighBits(), TEST_BITS[i] | TEST_BITS[j]);
                assert.equal(vi.or(vj).getLowBits(), TEST_BITS[i + 1] | TEST_BITS[j + 1]);
                assert.equal(vi.xor(vj).getHighBits(), TEST_BITS[i] ^ TEST_BITS[j]);
                assert.equal(vi.xor(vj).getLowBits(), TEST_BITS[i + 1] ^ TEST_BITS[j + 1]);
            }

            assert.equal(vi.shl(0).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shl(0).getLowBits(), TEST_BITS[i + 1]);
            assert.equal(vi.shr(0).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shr(0).getLowBits(), TEST_BITS[i + 1]);
            assert.equal(vi.shru(0).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shru(0).getLowBits(), TEST_BITS[i + 1]);
            ...
            for (let len = 1; len < 64; ++len) {
                if (len < 32) {
                    ...
                    assert.equal(vi.shr(len).getHighBits(), TEST_BITS[i] >= 0 ? 0 : -1);
                    assert.equal(vi.shr(len).getLowBits(), TEST_BITS[i] >> (len - 32));

                    assert.equal(vi.shru(len).getHighBits(), 0);
                    if (len === 32) {
                        assert.equal(vi.shru(len).getLowBits(), TEST_BITS[i]);
                    } else {
                        assert.equal(vi.shru(len).getLowBits(), TEST_BITS[i] >>> (len - 32));
                    }
                }
            }

            assert.equal(vi.shl(64).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shl(64).getLowBits(), TEST_BITS[i + 1]);
            assert.equal(vi.shr(64).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shr(64).getLowBits(), TEST_BITS[i + 1]);
            assert.equal(vi.shru(64).getHighBits(), TEST_BITS[i]);
            assert.equal(vi.shru(64).getLowBits(), TEST_BITS[i + 1]);
        }
    });
    ...
    it("negation", () => {
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            const vi = Long.fromBits(TEST_BITS[i + 1], TEST_BITS[i]);
            if (TEST_BITS[i + 1] === 0) {
                assert.equal(vi.negate().getHighBits(), (~TEST_BITS[i] + 1) | 0);
                assert.equal(vi.negate().getLowBits(), 0);
            } else {
                assert.equal(vi.negate().getHighBits(), ~TEST_BITS[i]);
                assert.equal(vi.negate().getLowBits(), (~TEST_BITS[i + 1] + 1) | 0);
            }
        }
    });
    ...
    it("add", () => {
        ...
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            for (let j = 0; j < i; j += 2) {
                ...
                assert.equal(result.getHighBits(), TEST_ADD_BITS[count++]);
                assert.equal(result.getLowBits(), TEST_ADD_BITS[count++]);
            }
        }
    });
    ...
    it("sub", () => {
        ...
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            for (let j = 0; j < TEST_BITS.length; j += 2) {
                const vj = Long.fromBits(TEST_BITS[j + 1], TEST_BITS[j]);
                const result = vi.sub(vj);
                assert.equal(result.getHighBits(), TEST_SUB_BITS[count++]);
                assert.equal(result.getLowBits(), TEST_SUB_BITS[count++]);
            }
        }
    });
    ...
    it("mul", () => {
        ...
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            for (let j = 0; j < i; j += 2) {
                const vj = Long.fromBits(TEST_BITS[j + 1], TEST_BITS[j]);
                const result = vi.mul(vj);
                assert.equal(result.getHighBits(), TEST_MUL_BITS[count++]);
                assert.equal(result.getLowBits(), TEST_MUL_BITS[count++]);
            }
        }
    });
    ...
    it("fromString", () => {
        for (let i = 0; i < TEST_BITS.length; i += 2) {
            ...
            assert.equal(str, TEST_STRINGS[i / 2]);
            assert.equal(Long.fromString(str, 10).getHighBits(), TEST_BITS[i]);
            assert.equal(Long.fromString(str, 10).getLowBits(), TEST_BITS[i + 1]);

            for (let radix = 2; radix <= 36; ++radix) {
                const result = vi.toString(radix);
                assert.equal(Long.fromString(result, radix).getHighBits(), TEST_BITS[i]);
                assert.equal(Long.fromString(result, radix).getLowBits(), TEST_BITS[i + 1]);
            }
        }
    });
