...
describe("datetime", "locale", "ne", () => {
    commonLocaleTests("ne");

    beforeEach(() => {
        ateos.datetime.locale("ne");
    });

    it("parse", () => {
        const tests = "जनवरी जन._फेब्रुवरी फेब्रु._मार्च मार्च_अप्रिल अप्रि._मई मई_जुन जुन_जुलाई जुलाई._अगष्ट अग._सेप्टेम्बर सेप्ट._अक्टोबर अक्टो._नोभेम्बर नोभे._डिसेम्बर डिसे.".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "जनवरी जन._फेब्रुवरी फेब्रु._मार्च मार्च_अप्रिल अप्रि._मई मई_जुन जुन_जुलाई जुलाई._अगष्ट अग._सेप्टेम्बर सेप्ट._अक्टोबर अक्टो._नोभेम्बर नोभे._डिसेम्बर डिसे.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "आइतबार आइत. आ._सोमबार सोम. सो._मङ्गलबार मङ्गल. मं._बुधबार बुध. बु._बिहिबार बिहि. बि._शुक्रबार शुक्र. शु._शनिबार शनि. श.".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "केही क्षण", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "एक मिनेट", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "एक मिनेट", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "२ मिनेट", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "४४ मिनेट", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "एक घण्टा", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "एक घण्टा", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "२ घण्टा", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "५ घण्टा", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "२१ घण्टा", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "एक महिना", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "एक महिना", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "एक महिना", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "२ महिना", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "२ महिना", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "३ महिना", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "एक महिना", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "५ महिना", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "एक बर्ष", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "२ बर्ष", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "एक बर्ष", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "५ बर्ष", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "केही क्षणमा", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "केही क्षण अगाडि", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "केही क्षण अगाडि", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "केही क्षणमा", "केही क्षणमा");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "५ दिनमा", "५ दिनमा");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "आज दिउँसोको १२:०० बजे", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "आज दिउँसोको १२:२५ बजे", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "आज दिउँसोको १:०० बजे", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "भोलि दिउँसोको १२:०० बजे", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "आज बिहानको ११:०० बजे", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "हिजो दिउँसोको १२:०० बजे", "yesterday at the same time");
    });
    ...
    it("meridiem", () => {
        assert.equal(ateos.datetime([2011, 2, 23, 2, 30]).format("a"), "राति", "before dawn");
        assert.equal(ateos.datetime([2011, 2, 23, 9, 30]).format("a"), "बिहान", "morning");
        assert.equal(ateos.datetime([2011, 2, 23, 14, 30]).format("a"), "दिउँसो", "during day");
        assert.equal(ateos.datetime([2011, 2, 23, 17, 30]).format("a"), "साँझ", "evening");
        assert.equal(ateos.datetime([2011, 2, 23, 19, 30]).format("a"), "साँझ", "late evening");
        assert.equal(ateos.datetime([2011, 2, 23, 21, 20]).format("a"), "राति", "night");
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(ateos.datetime([2011, 11, 26]).format("w ww wo"), "५३ ५३ ५३", "Dec 26 2011 should be week 53");
        assert.equal(ateos.datetime([2012, 0, 1]).format("w ww wo"), "१ ०१ १", "Jan  1 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 2]).format("w ww wo"), "१ ०१ १", "Jan  2 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 8]).format("w ww wo"), "२ ०२ २", "Jan  8 2012 should be week 2");
        assert.equal(ateos.datetime([2012, 0, 9]).format("w ww wo"), "२ ०२ २", "Jan  9 2012 should be week 2");
    });
});
