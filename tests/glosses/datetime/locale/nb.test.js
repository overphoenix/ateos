...
describe("datetime", "locale", "nb", () => {
    commonLocaleTests("nb");

    beforeEach(() => {
        ateos.datetime.locale("nb");
    });

    it("parse", () => {
        const tests = "januar jan._februar feb._mars mars_april april_mai mai_juni juni_juli juli_august aug._september sep._oktober okt._november nov._desember des.".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "januar jan._februar feb._mars mars_april april_mai mai_juni juni_juli juli_august aug._september sep._oktober okt._november nov._desember des.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "søndag sø. sø_mandag ma. ma_tirsdag ti. ti_onsdag on. on_torsdag to. to_fredag fr. fr_lørdag lø. lø".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "noen sekunder", "44 sekunder = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "ett minutt", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "ett minutt", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "2 dager", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "en dag", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "5 dager", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "25 dager", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "om noen sekunder", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "noen sekunder siden", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "noen sekunder siden", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "om noen sekunder", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "om 5 dager", "in 5 days");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "i dag kl. 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "i dag kl. 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "i morgen kl. 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "i dag kl. 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "i går kl. 12:00", "yesterday at the same time");
    });
