...
describe("datetime", "locale", "pt", () => {
    commonLocaleTests("pt");

    beforeEach(() => {
        ateos.datetime.locale("pt");
    });
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "segundos", "44 seconds = seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "em segundos", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "há segundos", "suffix");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "em segundos", "in seconds");
        assert.equal(ateos.datetime().add({
            d: 5
