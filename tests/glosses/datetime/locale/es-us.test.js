...
describe("datetime", "locale", "es-us", () => {
    commonLocaleTests("es-us");

    before(() => {
        ateos.datetime.locale("es-us");
    });
    ...
    it("parse", () => {
        let tests = "enero ene._febrero feb._marzo mar._abril abr._mayo may._junio jun._julio jul._agosto ago._septiembre sep._octubre oct._noviembre nov._diciembre dic.".split("_"), i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ s: 44 }), true), "unos segundos", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ s: 45 }), true), "un minuto", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ s: 89 }), true), "un minuto", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ s: 90 }), true), "2 minutos", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ m: 44 }), true), "44 minutos", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ m: 45 }), true), "una hora", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ m: 89 }), true), "una hora", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ m: 90 }), true), "2 horas", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ h: 5 }), true), "5 horas", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ h: 21 }), true), "21 horas", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ h: 22 }), true), "un día", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ h: 35 }), true), "un día", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ h: 36 }), true), "2 días", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 1 }), true), "un día", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 5 }), true), "5 días", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 25 }), true), "25 días", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 26 }), true), "un mes", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 30 }), true), "un mes", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 43 }), true), "un mes", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 46 }), true), "2 meses", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 74 }), true), "2 meses", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 76 }), true), "3 meses", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ M: 1 }), true), "un mes", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ M: 5 }), true), "5 meses", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 345 }), true), "un año", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ d: 548 }), true), "2 años", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ y: 1 }), true), "un año", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({ y: 5 }), true), "5 años", "5 years = 5 years");
    });
    ...
    it("fromNow", () => {
        assert.equal(ateos.datetime().add({ s: 30 }).fromNow(), "en unos segundos", "en unos segundos");
        assert.equal(ateos.datetime().add({ d: 5 }).fromNow(), "en 5 días", "en 5 días");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).add({ m: 25 }).calendar(), "hoy a las 12:25 PM", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({ h: 1 }).calendar(), "hoy a las 1:00 PM", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({ d: 1 }).calendar(), "mañana a las 12:00 PM", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).add({ d: 1, h: -1 }).calendar(), "mañana a las 11:00 AM", "tomorrow minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({ h: 1 }).calendar(), "hoy a las 11:00 AM", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({ d: 1 }).calendar(), "ayer a las 12:00 PM", "yesterday at the same time");
    });
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(ateos.datetime([2012, 0, 1]).format("w ww wo"), "1 01 1º", "Jan  1 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 2]).format("w ww wo"), "1 01 1º", "Jan  2 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 7]).format("w ww wo"), "1 01 1º", "Jan  7 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 8]).format("w ww wo"), "2 02 2º", "Jan  8 2012 should be week 2");
        assert.equal(ateos.datetime([2012, 0, 14]).format("w ww wo"), "2 02 2º", "Jan 14 2012 should be week 2");
        assert.equal(ateos.datetime([2012, 0, 15]).format("w ww wo"), "3 03 3º", "Jan 15 2012 should be week 3");
    });
});
