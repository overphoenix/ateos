...
describe("datetime", "locale", "br", () => {
    ...
    it("format ordinal", () => {
        ateos.datetime.locale("br");
        assert.equal(ateos.datetime([2011, 0, 1]).format("DDDo"), "1añ", "1añ");
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "un nebeud segondennoù", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
    ...
    it("suffix", () => {
        ateos.datetime.locale("br");
        assert.equal(ateos.datetime(30000).from(0), "a-benn un nebeud segondennoù", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "un nebeud segondennoù 'zo", "suffix");
    });

    it("now from now", () => {
        ateos.datetime.locale("br");
        assert.equal(ateos.datetime().fromNow(), "un nebeud segondennoù 'zo", "now from now should display as in the past");
    });
    ...
    it("fromNow", () => {
        ...
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "a-benn un nebeud segondennoù", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
