...
describe("datetime", "locale", "vi", () => {
    ...
    it("parse", () => {
        ...
        function equalTest(input, mmm, i) {
            assert.equal(ateos.datetime(input, mmm).month(), i, `${input} should be month ${i}`);
        }
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "vài giây tới", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "vài giây trước", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "vài giây trước", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "Hôm nay lúc 12:00", "today at the same time");
