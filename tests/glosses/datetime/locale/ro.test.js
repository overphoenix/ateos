...
describe("datetime", "locale", "ro", () => {
    commonLocaleTests("ro");

    beforeEach(() => {
        ateos.datetime.locale("ro");
    });

    it("parse", () => {
        const tests = "ianuarie ian._februarie febr._martie mart._aprilie apr._mai mai_iunie iun._iulie iul._august aug._septembrie sept._octombrie oct._noiembrie nov._decembrie dec.".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "ianuarie ian._februarie febr._martie mart._aprilie apr._mai mai_iunie iun._iulie iul._august aug._septembrie sept._octombrie oct._noiembrie nov._decembrie dec.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "duminică Dum Du_luni Lun Lu_marți Mar Ma_miercuri Mie Mi_joi Joi Jo_vineri Vin Vi_sâmbătă Sâm Sâ".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "câteva secunde", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "2 minute", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "44 de minute", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "o oră", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "o oră", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "2 ore", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "5 ore", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "21 de ore", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "o zi", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "o zi", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "2 zile", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "o zi", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "5 zile", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "25 de zile", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "o lună", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "o lună", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "o lună", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "2 luni", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "2 luni", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "3 luni", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "o lună", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "5 luni", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "un an", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "2 ani", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "un an", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "5 ani", "5 years = 5 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 19
        }), true), "19 ani", "19 years = 19 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 20
        }), true), "20 de ani", "20 years = 20 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 100
        }), true), "100 de ani", "100 years = 100 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 101
        }), true), "101 ani", "101 years = 101 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 119
        }), true), "119 ani", "119 years = 119 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 120
        }), true), "120 de ani", "120 years = 120 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 219
        }), true), "219 ani", "219 years = 219 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 220
        }), true), "220 de ani", "220 years = 220 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "peste câteva secunde", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "câteva secunde în urmă", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "câteva secunde în urmă", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "peste câteva secunde", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "peste 5 zile", "in 5 days");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "azi la 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "azi la 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "azi la 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "mâine la 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "azi la 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "ieri la 12:00", "yesterday at the same time");
    });
