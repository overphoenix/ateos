...
describe("datetime", "locale", "ru", () => {
    ...
    it("parse", () => {
        ...
        function equalTestStrict(input, mmm, monthIndex) {
            assert.equal(ateos.datetime(input, mmm, true).month(), monthIndex, `${input} ${mmm} should be strict month ${monthIndex + 1}`);
        }
        for (i = 0; i < 12; i++) {
    ...
    it("parse exceptional case", () => {
        assert.equal(ateos.datetime("11 Мая 1989", ["DD MMMM YYYY"]).format("DD-MM-YYYY"), "11-05-1989");
    });
    ...
    it("format meridiem", () => {
        assert.equal(ateos.datetime([2012, 11, 28, 0, 0]).format("A"), "ночи", "night");
        assert.equal(ateos.datetime([2012, 11, 28, 3, 59]).format("A"), "ночи", "night");
        assert.equal(ateos.datetime([2012, 11, 28, 4, 0]).format("A"), "утра", "morning");
        assert.equal(ateos.datetime([2012, 11, 28, 11, 59]).format("A"), "утра", "morning");
        ...
        assert.equal(ateos.datetime([2012, 11, 28, 17, 0]).format("A"), "вечера", "evening");
        assert.equal(ateos.datetime([2012, 11, 28, 23, 59]).format("A"), "вечера", "evening");
    });
    ...
    it("format month case with escaped symbols", () => {
        ...
        for (i = 0; i < 12; i++) {
            ...
            assert.equal(ateos.datetime([2013, i, 1]).format("D[-й день] MMMM"), `1-й день ${months.accusative[i]}`, `1-й день ${months.accusative[i]}`);
    ...
    it("format month short case with escaped symbols", () => {
        ...
        for (i = 0; i < 12; i++) {
            ...
            assert.equal(ateos.datetime([2013, i, 1]).format("D[-й день] MMM"), `1-й день ${monthsShort.accusative[i]}`, `1-й день ${monthsShort.accusative[i]}`);
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "через несколько секунд", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "несколько секунд назад", "suffix");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "Сегодня в 12:00", "today at the same time");
    ...
    it("calendar next week", () => {
        ...
        now = ateos.datetime().startOf("week");
        for (i = 2; i < 7; i++) {
            m = ateos.datetime(now).add({
                d: i
            });
    ...
    it("weeks year starting monday formatted", () => {
        assert.equal(ateos.datetime([2011, 11, 26]).format("w ww wo"), "52 52 52-я", "Dec 26 2011 should be week 52");
        assert.equal(ateos.datetime([2012, 0, 1]).format("w ww wo"), "52 52 52-я", "Jan  1 2012 should be week 52");
        assert.equal(ateos.datetime([2012, 0, 2]).format("w ww wo"), "1 01 1-я", "Jan  2 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 8]).format("w ww wo"), "1 01 1-я", "Jan  8 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 9]).format("w ww wo"), "2 02 2-я", "Jan  9 2012 should be week 2");
    });
});
