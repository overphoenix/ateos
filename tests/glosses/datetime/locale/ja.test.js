...
describe("datetime", "locale", "ja", () => {
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "数秒後", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "数秒前", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "数秒前", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "今日 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({ m: 25 }).calendar(), "今日 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({ h: 1 }).calendar(), "今日 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({ d: 1 }).calendar(), "明日 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({ h: 1 }).calendar(), "今日 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({ d: 1 }).calendar(), "昨日 12:00", "yesterday at the same time");
    });
    ...
    it("parse with japanese parentheses", () => {
        assert.ok(ateos.datetime("2016年5月18日（水）", "YYYY年M月D日（dd）", true).isValid(), "parse with japanese parentheses");
    });
});
