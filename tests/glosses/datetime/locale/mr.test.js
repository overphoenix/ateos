...
describe("datetime", "locale", "mr", () => {
    commonLocaleTests("mr");

    beforeEach(() => {
        ateos.datetime.locale("mr");
    });

    it("parse", () => {
        const tests = "जानेवारी जाने._फेब्रुवारी फेब्रु._मार्च मार्च._एप्रिल एप्रि._मे मे._जून जून._जुलै जुलै._ऑगस्ट ऑग._सप्टेंबर सप्टें._ऑक्टोबर ऑक्टो._नोव्हेंबर नोव्हें._डिसेंबर डिसें.".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "जानेवारी जाने._फेब्रुवारी फेब्रु._मार्च मार्च._एप्रिल एप्रि._मे मे._जून जून._जुलै जुलै._ऑगस्ट ऑग._सप्टेंबर सप्टें._ऑक्टोबर ऑक्टो._नोव्हेंबर नोव्हें._डिसेंबर डिसें.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "रविवार रवि र_सोमवार सोम सो_मंगळवार मंगळ मं_बुधवार बुध बु_गुरूवार गुरू गु_शुक्रवार शुक्र शु_शनिवार शनि श".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "काही सेकंद", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "एक मिनिट", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "एक मिनिट", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "२ मिनिटे", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "४४ मिनिटे", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "एक तास", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "एक तास", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "२ तास", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "५ तास", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "२१ तास", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "एक दिवस", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "एक दिवस", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "२ दिवस", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "एक दिवस", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "५ दिवस", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "२५ दिवस", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "२ महिने", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "२ महिने", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "३ महिने", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "एक महिना", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "५ महिने", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "एक वर्ष", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "२ वर्षे", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "एक वर्ष", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "५ वर्षे", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "काही सेकंदांमध्ये", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "काही सेकंदांपूर्वी", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "काही सेकंदांपूर्वी", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "काही सेकंदांमध्ये", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "५ दिवसांमध्ये", "in 5 days");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "आज दुपारी १२:०० वाजता", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "आज दुपारी १२:२५ वाजता", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 3
        }).calendar(), "आज दुपारी ३:०० वाजता", "Now plus 3 hours");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "उद्या दुपारी १२:०० वाजता", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "आज दुपारी ११:०० वाजता", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "काल दुपारी १२:०० वाजता", "yesterday at the same time");
    });
    ...
    it("meridiem", () => {
        assert.equal(ateos.datetime([2011, 2, 23, 2, 30]).format("a"), "रात्री", "before dawn");
        assert.equal(ateos.datetime([2011, 2, 23, 9, 30]).format("a"), "सकाळी", "morning");
        assert.equal(ateos.datetime([2011, 2, 23, 14, 30]).format("a"), "दुपारी", "during day");
        assert.equal(ateos.datetime([2011, 2, 23, 17, 30]).format("a"), "सायंकाळी", "evening");
        assert.equal(ateos.datetime([2011, 2, 23, 19, 30]).format("a"), "सायंकाळी", "late evening");
        assert.equal(ateos.datetime([2011, 2, 23, 21, 20]).format("a"), "रात्री", "night");
