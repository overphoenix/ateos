...
describe("datetime", "locale", "sr", () => {
    commonLocaleTests("sr");

    beforeEach(() => {
        ateos.datetime.locale("sr");
    });
    ...
    it("format week", () => {
        const expected = "nedelja ned. ne_ponedeljak pon. po_utorak uto. ut_sreda sre. sr_četvrtak čet. če_petak pet. pe_subota sub. su".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "mesec", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "mesec", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "mesec", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "3 meseca", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "mesec", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "5 meseci", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
    ...
    it("suffix", () => {
        ...
        assert.equal(ateos.datetime(0).from(30000), "pre nekoliko sekundi", "prefix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "pre nekoliko sekundi", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "juče u 12:00", "yesterday at the same time");
    });
    ...
    it("calendar last week", () => {
        ...
        function makeFormat(d) {
            const lastWeekDay = ["[prošle] [nedelje] [u] LT", "[prošlog] [ponedeljka] [u] LT", "[prošlog] [utorka] [u] LT", "[prošle] [srede] [u] LT", "[prošlog] [četvrtka] [u] LT", "[prošlog] [petka] [u] LT", "[prošle] [subote] [u] LT"];
