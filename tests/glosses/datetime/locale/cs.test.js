...
describe("datetime", "locale", "cs", () => {
    commonLocaleTests("cs");

    before(() => {
        ateos.datetime.locale("cs");
    });
    ...
    it("parse", () => {
        const tests = "leden led_únor úno_březen bře_duben dub_květen kvě_červen čvn_červenec čvc_srpen srp_září zář_říjen říj_listopad lis_prosinec pro".split("_");
        let i;

        function equalTest(input, mmm, monthIndex) {
            assert.equal(ateos.datetime(input, mmm).month(), monthIndex, `${input} ${mmm} should be month ${monthIndex + 1}`);
        }
    ...
    it("format month", () => {
        const expected = "leden led_únor úno_březen bře_duben dub_květen kvě_červen čvn_červenec čvc_srpen srp_září zář_říjen říj_listopad lis_prosinec pro".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "neděle ne ne_pondělí po po_úterý út út_středa st st_čtvrtek čt čt_pátek pá pá_sobota so so".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "pár sekund", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "minuta", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "minuta", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "2 minuty", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "44 minut", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "hodina", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "hodina", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "2 hodiny", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "5 hodin", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "21 hodin", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "den", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "den", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "2 dny", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "den", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "5 dní", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "25 dní", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "měsíc", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "měsíc", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "měsíc", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "2 měsíce", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "2 měsíce", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "3 měsíce", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "měsíc", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "5 měsíců", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "rok", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "2 roky", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "rok", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "za pár sekund", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "před pár sekundami", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "před pár sekundami", "now from now should display as in the past");
    });

    it("fromNow (future)", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "za pár sekund", "in a few seconds");
        assert.equal(ateos.datetime().add({
            m: 1
        }).fromNow(), "za minutu", "in a minute");
        assert.equal(ateos.datetime().add({
            m: 3
        }).fromNow(), "za 3 minuty", "in 3 minutes");
        assert.equal(ateos.datetime().add({
            m: 10
        }).fromNow(), "za 10 minut", "in 10 minutes");
        assert.equal(ateos.datetime().add({
            h: 1
        }).fromNow(), "za hodinu", "in an hour");
        assert.equal(ateos.datetime().add({
            h: 3
        }).fromNow(), "za 3 hodiny", "in 3 hours");
        assert.equal(ateos.datetime().add({
            h: 10
        }).fromNow(), "za 10 hodin", "in 10 hours");
        assert.equal(ateos.datetime().add({
            d: 1
        }).fromNow(), "za den", "in a day");
        assert.equal(ateos.datetime().add({
            d: 3
        }).fromNow(), "za 3 dny", "in 3 days");
        assert.equal(ateos.datetime().add({
            d: 10
        }).fromNow(), "za 10 dní", "in 10 days");
        assert.equal(ateos.datetime().add({
            M: 1
        }).fromNow(), "za měsíc", "in a month");
        assert.equal(ateos.datetime().add({
            M: 3
        }).fromNow(), "za 3 měsíce", "in 3 months");
        assert.equal(ateos.datetime().add({
            M: 10
        }).fromNow(), "za 10 měsíců", "in 10 months");
        assert.equal(ateos.datetime().add({
            y: 1
        }).fromNow(), "za rok", "in a year");
        assert.equal(ateos.datetime().add({
            y: 3
        }).fromNow(), "za 3 roky", "in 3 years");
        assert.equal(ateos.datetime().add({
            y: 10
        }).fromNow(), "za 10 let", "in 10 years");
    });

    it("fromNow (past)", () => {
        assert.equal(ateos.datetime().subtract({
            s: 30
        }).fromNow(), "před pár sekundami", "a few seconds ago");
        assert.equal(ateos.datetime().subtract({
            m: 1
        }).fromNow(), "před minutou", "a minute ago");
        assert.equal(ateos.datetime().subtract({
            m: 3
        }).fromNow(), "před 3 minutami", "3 minutes ago");
        assert.equal(ateos.datetime().subtract({
            m: 10
        }).fromNow(), "před 10 minutami", "10 minutes ago");
        assert.equal(ateos.datetime().subtract({
            h: 1
        }).fromNow(), "před hodinou", "an hour ago");
        assert.equal(ateos.datetime().subtract({
            h: 3
        }).fromNow(), "před 3 hodinami", "3 hours ago");
        assert.equal(ateos.datetime().subtract({
            h: 10
        }).fromNow(), "před 10 hodinami", "10 hours ago");
        assert.equal(ateos.datetime().subtract({
            d: 1
        }).fromNow(), "před dnem", "a day ago");
        assert.equal(ateos.datetime().subtract({
            d: 3
        }).fromNow(), "před 3 dny", "3 days ago");
        assert.equal(ateos.datetime().subtract({
            d: 10
        }).fromNow(), "před 10 dny", "10 days ago");
        assert.equal(ateos.datetime().subtract({
            M: 1
        }).fromNow(), "před měsícem", "a month ago");
        assert.equal(ateos.datetime().subtract({
            M: 3
        }).fromNow(), "před 3 měsíci", "3 months ago");
        assert.equal(ateos.datetime().subtract({
            M: 10
        }).fromNow(), "před 10 měsíci", "10 months ago");
        assert.equal(ateos.datetime().subtract({
            y: 1
        }).fromNow(), "před rokem", "a year ago");
        assert.equal(ateos.datetime().subtract({
            y: 3
        }).fromNow(), "před 3 lety", "3 years ago");
        assert.equal(ateos.datetime().subtract({
            y: 10
        }).fromNow(), "před 10 lety", "10 years ago");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "dnes v 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "dnes v 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "dnes v 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "zítra v 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "dnes v 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "včera v 12:00", "yesterday at the same time");
    });
    ...
    it("humanize duration", () => {
        assert.equal(ateos.datetime.duration(1, "minutes").humanize(), "minuta", "a minute (future)");
        assert.equal(ateos.datetime.duration(1, "minutes").humanize(true), "za minutu", "in a minute");
        assert.equal(ateos.datetime.duration(-1, "minutes").humanize(), "minuta", "a minute (past)");
        assert.equal(ateos.datetime.duration(-1, "minutes").humanize(true), "před minutou", "a minute ago");
    });
