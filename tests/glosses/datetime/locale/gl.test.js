...
describe("datetime", "locale", "gl", () => {
    commonLocaleTests("gl");

    beforeEach(() => {
        ateos.datetime.locale("gl");
    });
    ...
    it("format month", () => {
        const expected = "xaneiro xan._febreiro feb._marzo mar._abril abr._maio mai._xuño xuñ._xullo xul._agosto ago._setembro set._outubro out._novembro nov._decembro dec.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "domingo dom. do_luns lun. lu_martes mar. ma_mércores mér. mé_xoves xov. xo_venres ven. ve_sábado sáb. sá".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "uns segundos", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "unha hora", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "unha hora", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "un ano", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "2 anos", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "un ano", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "nuns segundos", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "hai uns segundos", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "hai uns segundos", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "nuns segundos", "nuns segundos");
        assert.equal(ateos.datetime().add({
            d: 5
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "hoxe ás 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "hoxe ás 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "hoxe ás 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "mañá ás 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).add({
            d: 1,
            h: -1
        }).calendar(), "mañá ás 11:00", "tomorrow minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "hoxe ás 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "onte á 12:00", "yesterday at the same time");
    });
    ...
    it("regression tests", () => {
        const lastWeek = ateos.datetime().subtract({
            d: 4
        }).hours(1);
