...
describe("datetime", "locale", "km", () => {
    commonLocaleTests("km");

    beforeEach(() => {
        ateos.datetime.locale("km");
    });

    it("parse", () => {
        const tests = "មករា មករា_កុម្ភៈ កុម្ភៈ_មីនា មីនា_មេសា មេសា_ឧសភា ឧសភា_មិថុនា មិថុនា_កក្កដា កក្កដា_សីហា សីហា_កញ្ញា កញ្ញា_តុលា តុលា_វិច្ឆិកា វិច្ឆិកា_ធ្នូ ធ្នូ".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "មករា មករា_កុម្ភៈ កុម្ភៈ_មីនា មីនា_មេសា មេសា_ឧសភា ឧសភា_មិថុនា មិថុនា_កក្កដា កក្កដា_សីហា សីហា_កញ្ញា កញ្ញា_តុលា តុលា_វិច្ឆិកា វិច្ឆិកា_ធ្នូ ធ្នូ".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "អាទិត្យ អាទិត្យ អាទិត្យ_ច័ន្ទ ច័ន្ទ ច័ន្ទ_អង្គារ អង្គារ អង្គារ_ពុធ ពុធ ពុធ_ព្រហស្បតិ៍ ព្រហស្បតិ៍ ព្រហស្បតិ៍_សុក្រ សុក្រ សុក្រ_សៅរ៍ សៅរ៍ សៅរ៍".split("_");
        let i;
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "ប៉ុន្មានវិនាទីទៀត", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "ប៉ុន្មានវិនាទីមុន", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "ប៉ុន្មានវិនាទីមុន", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "ថ្ងៃនេះ ម៉ោង 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "ថ្ងៃនេះ ម៉ោង 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "ថ្ងៃនេះ ម៉ោង 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "ស្អែក ម៉ោង 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "ថ្ងៃនេះ ម៉ោង 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "ម្សិលមិញ ម៉ោង 12:00", "yesterday at the same time");
    });
