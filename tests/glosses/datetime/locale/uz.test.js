...
describe("datetime", "locale", "uz", () => {
    commonLocaleTests("uz");

    beforeEach(() => {
        ateos.datetime.locale("uz");
    });

    it("parse", () => {
        const tests = "январ янв_феврал фев_март мар_апрел апр_май май_июн июн_июл июл_август авг_сентябр сен_октябр окт_ноябр ноя_декабр дек".split("_");
        let i;
    ...
    it("format month", () => {
        const expected = "январ янв_феврал фев_март мар_апрел апр_май май_июн июн_июл июл_август авг_сентябр сен_октябр окт_ноябр ноя_декабр дек".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "Якшанба Якш Як_Душанба Душ Ду_Сешанба Сеш Се_Чоршанба Чор Чо_Пайшанба Пай Па_Жума Жум Жу_Шанба Шан Ша".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "фурсат", "44 секунд = фурсат");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "бир дакика", "45 секунд = бир дакика");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "бир дакика", "89 секунд = бир дакика");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "2 дакика", "90 секунд = 2 дакика");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "44 дакика", "44 дакика = 44 дакика");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "бир соат", "45 минут = бир соат");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "бир соат", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "2 соат", "90 минут = 2 соат");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "5 соат", "5 соат = 5 соат");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "21 соат", "21 соат = 21 соат");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "бир кун", "22 соат = бир кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "бир кун", "35 соат = бир кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "2 кун", "36 соат = 2 кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "бир кун", "1 кун = 1 кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "5 кун", "5 кун = 5 кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "25 кун", "25 кун = 25 кун");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "бир ой", "26 кун = бир ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "бир ой", "30 кун = бир ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "бир ой", "45 кун = бир ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "2 ой", "46 кун = 2 ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "2 ой", "75 кун = 2 ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "3 ой", "76 кун = 3 ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "бир ой", "бир ой = бир ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "5 ой", "5 ой = 5 ой");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "бир йил", "345 кун = бир йил");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "2 йил", "548 кун = 2 йил");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "бир йил", "1 йил = бир йил");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "5 йил", "5 йил = 5 йил");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "Якин фурсат ичида", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "Бир неча фурсат олдин", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "Бир неча фурсат олдин", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "Якин фурсат ичида", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "Якин 5 кун ичида", "in 5 days");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "Бугун соат 12:00 да", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "Бугун соат 12:25 да", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "Бугун соат 13:00 да", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "Эртага 12:00 да", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "Бугун соат 11:00 да", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "Кеча соат 12:00 да", "yesterday at the same time");
    });
