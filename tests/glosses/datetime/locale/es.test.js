...
describe("datetime", "locale", "es", () => {
    commonLocaleTests("es");
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "hoy a las 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "hoy a las 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "hoy a las 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "mañana a las 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).add({
            d: 1,
            h: -1
        }).calendar(), "mañana a las 11:00", "tomorrow minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "hoy a las 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "ayer a las 12:00", "yesterday at the same time");
    });
