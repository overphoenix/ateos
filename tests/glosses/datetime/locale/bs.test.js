...
describe("datetime", "locale", "bs", () => {
    commonLocaleTests("bs");

    before(() => {
        ateos.datetime.locale("bs");
    });
    ...
    it("parse", () => {
        const tests = "januar jan._februar feb._mart mar._april apr._maj maj._juni jun._juli jul._august aug._septembar sep._oktobar okt._novembar nov._decembar dec.".split("_");
        let i;

        function equalTest(input, mmm, i) {
            assert.equal(ateos.datetime(input, mmm).month(), i, `${input} should be month ${i + 1} inp ${mmm}`);
        }
        for (i = 0; i < 12; i++) {
    ...
    it("format month", () => {
        const expected = "januar jan._februar feb._mart mar._april apr._maj maj._juni jun._juli jul._august aug._septembar sep._oktobar okt._novembar nov._decembar dec.".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "nedjelja ned. ne_ponedjeljak pon. po_utorak uto. ut_srijeda sri. sr_četvrtak čet. če_petak pet. pe_subota sub. su".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "par sekundi", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "jedna minuta", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "jedna minuta", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "za par sekundi", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "prije par sekundi", "prefix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "prije par sekundi", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "za par sekundi", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "sutra u 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "danas u 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "jučer u 12:00", "yesterday at the same time");
    });
