...
describe("datetime", "locale", "fr-ca", () => {
    commonLocaleTests("fr-ca");

    beforeEach(() => {
        ateos.datetime.locale("fr-ca");
    });
    ...
    it("weeks year starting sunday formatted", () => {
        assert.equal(ateos.datetime([2012, 0, 1]).format("w ww wo"), "1 01 1re", "Jan  1 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 7]).format("w ww wo"), "1 01 1re", "Jan  7 2012 should be week 1");
        assert.equal(ateos.datetime([2012, 0, 8]).format("w ww wo"), "2 02 2e", "Jan  8 2012 should be week 2");
        assert.equal(ateos.datetime([2012, 0, 14]).format("w ww wo"), "2 02 2e", "Jan 14 2012 should be week 2");
        assert.equal(ateos.datetime([2012, 0, 15]).format("w ww wo"), "3 03 3e", "Jan 15 2012 should be week 3");
    });
});
