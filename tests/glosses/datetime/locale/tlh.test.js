...
describe("datetime", "locale", "tlh", () => {
    commonLocaleTests("tlh");

    beforeEach(() => {
        ateos.datetime.locale("tlh");
    });
    ...
    */
    ...
    it("format week", () => {
        const expected = "lojmItjaj lojmItjaj lojmItjaj_DaSjaj DaSjaj DaSjaj_povjaj povjaj povjaj_ghItlhjaj ghItlhjaj ghItlhjaj_loghjaj loghjaj loghjaj_buqjaj buqjaj buqjaj_ghInjaj ghInjaj ghInjaj".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "puS lup", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "wa’ tup", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "wa’ tup", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "cha’ tup", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "loSmaH loS tup", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        }), true), "wa’ rep", "45 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 89
        }), true), "wa’ rep", "89 minutes = an hour");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 90
        }), true), "cha’ rep", "90 minutes = 2 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "vagh rep", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "cha’maH wa’ rep", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "wa’ jaj", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "wa’ jaj", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "cha’ jaj", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "wa’ jaj", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        }), true), "vagh jaj", "5 days = 5 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 25
        }), true), "cha’maH vagh jaj", "25 days = 25 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "wa’ jar", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "wa’ jar", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "wa’ jar", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "cha’ jar", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "cha’ jar", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "wej jar", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "wa’ jar", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "vagh jar", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        }), true), "wa’ DIS", "345 days = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 548
        }), true), "cha’ DIS", "548 days = 2 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 1
        }), true), "wa’ DIS", "1 year = a year");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "vagh DIS", "5 years = 5 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 112
        }), true), "wa’vatlh wa’maH cha’ DIS", "112 years = 112 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 122
        }), true), "wa’vatlh cha’maH cha’ DIS", "122 years = 122 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 213
        }), true), "cha’vatlh wa’maH wej DIS", "213 years = 213 years");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 223
        }), true), "cha’vatlh cha’maH wej DIS", "223 years = 223 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "puS lup pIq", "suffix");
        assert.equal(ateos.datetime(0).from(30000), "puS lup ret", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "puS lup ret", "now from now should display as in the past");
    });

    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "puS lup pIq", "in a few seconds");
        assert.equal(ateos.datetime().add({
            h: 1
        }).fromNow(), "wa’ rep pIq", "in an hour");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "vagh leS", "in 5 days");
        assert.equal(ateos.datetime().add({
            M: 2
        }).fromNow(), "cha’ waQ", "in 2 months");
        assert.equal(ateos.datetime().add({
            y: 1
        }).fromNow(), "wa’ nem", "in a year");
        assert.equal(ateos.datetime().add({
            s: -30
        }).fromNow(), "puS lup ret", "a few seconds ago");
        assert.equal(ateos.datetime().add({
            h: -1
        }).fromNow(), "wa’ rep ret", "an hour ago");
        assert.equal(ateos.datetime().add({
            d: -5
        }).fromNow(), "vagh Hu’", "5 days ago");
        assert.equal(ateos.datetime().add({
            M: -2
        }).fromNow(), "cha’ wen", "2 months ago");
        assert.equal(ateos.datetime().add({
            y: -1
        }).fromNow(), "wa’ ben", "a year ago");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "DaHjaj 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "DaHjaj 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "DaHjaj 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "wa’leS 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "DaHjaj 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "wa’Hu’ 12:00", "yesterday at the same time");
    });
