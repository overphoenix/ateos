...
describe("datetime", "locale", "ar-ly", () => {
    commonLocaleTests("ar-ly");

    before(() => {
        ateos.datetime.locale("ar-ly");
    });
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "44 ثانية", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "44 دقيقة", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "21 ساعة", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "5 أعوام", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "بعد 30 ثانية", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "منذ 30 ثانية", "suffix");
    });
    ...
    it("fromNow", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "بعد 30 ثانية", "in a few seconds");
        assert.equal(ateos.datetime().add({
            d: 5
        }).fromNow(), "بعد 5 أيام", "in 5 days");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "اليوم عند الساعة 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "اليوم عند الساعة 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "اليوم عند الساعة 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "غدًا عند الساعة 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "اليوم عند الساعة 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "أمس عند الساعة 12:00", "yesterday at the same time");
    });
    ...
    it("weeks year starting wednesday custom", () => {
        assert.equal(ateos.datetime("2003 1 6", "gggg w d").format("YYYY-MM-DD"), "2002-12-28", "Week 1 of 2003 should be Dec 28 2002");
        assert.equal(ateos.datetime("2003 1 0", "gggg w e").format("YYYY-MM-DD"), "2002-12-28", "Week 1 of 2003 should be Dec 28 2002");
        assert.equal(ateos.datetime("2003 1 6", "gggg w d").format("gggg w d"), "2003 1 6", "Saturday of week 1 of 2003 parsed should be formatted as 2003 1 6");
        assert.equal(ateos.datetime("2003 1 0", "gggg w e").format("gggg w e"), "2003 1 0", "1st day of week 1 of 2003 parsed should be formatted as 2003 1 0");
    });
    ...
    it("ar-ly strict mode parsing works", () => {
        const m = ateos.datetime().locale("ar-ly");
        const formattedDate = m.format("l");
        assert.equal(ateos.datetime.utc(formattedDate, "l", "ar-ly", false).isValid(), true, "Non-strict parsing works");
        assert.equal(ateos.datetime.utc(formattedDate, "l", "ar-ly", true).isValid(), true, "Strict parsing must work");
    });
});
