...
describe("datetime", "locale", "sk", () => {
    commonLocaleTests("sk");

    beforeEach(() => {
        ateos.datetime.locale("sk");
    });

    it("parse", () => {
        const tests = "január jan._február feb._marec mar._apríl apr._máj máj_jún jún._júl júl._august aug._september sep._október okt._november nov._december dec.".split("_");
        let i;

        function equalTest(input, mmm, monthIndex) {
            assert.equal(ateos.datetime(input, mmm).month(), monthIndex, `${input} should be month ${monthIndex + 1}`);
        }
        for (i = 0; i < 12; i++) {
    ...
    it("format month", () => {
        const expected = "január jan_február feb_marec mar_apríl apr_máj máj_jún jún_júl júl_august aug_september sep_október okt_november nov_december dec".split("_");
        let i;
    ...
    it("format week", () => {
        const expected = "nedeľa ne ne_pondelok po po_utorok ut ut_streda st st_štvrtok št št_piatok pi pi_sobota so so".split("_");
        let i;
    ...
    it("from", () => {
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 44
        }), true), "pár sekúnd", "44 seconds = a few seconds");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 45
        }), true), "minúta", "45 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 89
        }), true), "minúta", "89 seconds = a minute");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            s: 90
        }), true), "2 minúty", "90 seconds = 2 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 44
        }), true), "44 minút", "44 minutes = 44 minutes");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            m: 45
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 5
        }), true), "5 hodín", "5 hours = 5 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 21
        }), true), "21 hodín", "21 hours = 21 hours");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 22
        }), true), "deň", "22 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 35
        }), true), "deň", "35 hours = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            h: 36
        }), true), "2 dni", "36 hours = 2 days");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 1
        }), true), "deň", "1 day = a day");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 5
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 26
        }), true), "mesiac", "26 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 30
        }), true), "mesiac", "30 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 43
        }), true), "mesiac", "43 days = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 46
        }), true), "2 mesiace", "46 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 74
        }), true), "2 mesiace", "75 days = 2 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 76
        }), true), "3 mesiace", "76 days = 3 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 1
        }), true), "mesiac", "1 month = a month");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            M: 5
        }), true), "5 mesiacov", "5 months = 5 months");
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            d: 345
        ...
        assert.equal(start.from(ateos.datetime([2007, 1, 28]).add({
            y: 5
        }), true), "5 rokov", "5 years = 5 years");
    });

    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "za pár sekúnd", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "pred pár sekundami", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "pred pár sekundami", "now from now should display as in the past");
    });

    it("fromNow (future)", () => {
        assert.equal(ateos.datetime().add({
            s: 30
        }).fromNow(), "za pár sekúnd", "in a few seconds");
        assert.equal(ateos.datetime().add({
            m: 1
        }).fromNow(), "za minútu", "in a minute");
        assert.equal(ateos.datetime().add({
            m: 3
        }).fromNow(), "za 3 minúty", "in 3 minutes");
        assert.equal(ateos.datetime().add({
            m: 10
        }).fromNow(), "za 10 minút", "in 10 minutes");
        assert.equal(ateos.datetime().add({
            h: 1
        ...
        assert.equal(ateos.datetime().add({
            h: 10
        }).fromNow(), "za 10 hodín", "in 10 hours");
        assert.equal(ateos.datetime().add({
            d: 1
        }).fromNow(), "za deň", "in a day");
        assert.equal(ateos.datetime().add({
            d: 3
        }).fromNow(), "za 3 dni", "in 3 days");
        assert.equal(ateos.datetime().add({
            d: 10
        }).fromNow(), "za 10 dní", "in 10 days");
        assert.equal(ateos.datetime().add({
            M: 1
        }).fromNow(), "za mesiac", "in a month");
        assert.equal(ateos.datetime().add({
            M: 3
        }).fromNow(), "za 3 mesiace", "in 3 months");
        assert.equal(ateos.datetime().add({
            M: 10
        }).fromNow(), "za 10 mesiacov", "in 10 months");
        assert.equal(ateos.datetime().add({
            y: 1
        ...
        assert.equal(ateos.datetime().add({
            y: 10
        }).fromNow(), "za 10 rokov", "in 10 years");
    });

    it("fromNow (past)", () => {
        assert.equal(ateos.datetime().subtract({
            s: 30
        }).fromNow(), "pred pár sekundami", "a few seconds ago");
        assert.equal(ateos.datetime().subtract({
            m: 1
        }).fromNow(), "pred minútou", "a minute ago");
        assert.equal(ateos.datetime().subtract({
            m: 3
        }).fromNow(), "pred 3 minútami", "3 minutes ago");
        assert.equal(ateos.datetime().subtract({
            m: 10
        }).fromNow(), "pred 10 minútami", "10 minutes ago");
        assert.equal(ateos.datetime().subtract({
            h: 1
        }).fromNow(), "pred hodinou", "an hour ago");
        assert.equal(ateos.datetime().subtract({
            h: 3
        }).fromNow(), "pred 3 hodinami", "3 hours ago");
        assert.equal(ateos.datetime().subtract({
            h: 10
        }).fromNow(), "pred 10 hodinami", "10 hours ago");
        assert.equal(ateos.datetime().subtract({
            d: 1
        }).fromNow(), "pred dňom", "a day ago");
        assert.equal(ateos.datetime().subtract({
            d: 3
        }).fromNow(), "pred 3 dňami", "3 days ago");
        assert.equal(ateos.datetime().subtract({
            d: 10
        }).fromNow(), "pred 10 dňami", "10 days ago");
        assert.equal(ateos.datetime().subtract({
            M: 1
        }).fromNow(), "pred mesiacom", "a month ago");
        assert.equal(ateos.datetime().subtract({
            M: 3
        }).fromNow(), "pred 3 mesiacmi", "3 months ago");
        assert.equal(ateos.datetime().subtract({
            M: 10
        }).fromNow(), "pred 10 mesiacmi", "10 months ago");
        assert.equal(ateos.datetime().subtract({
            y: 1
        }).fromNow(), "pred rokom", "a year ago");
        assert.equal(ateos.datetime().subtract({
            y: 3
        }).fromNow(), "pred 3 rokmi", "3 years ago");
        assert.equal(ateos.datetime().subtract({
            y: 10
        }).fromNow(), "pred 10 rokmi", "10 years ago");
    });

    it("calendar day", () => {
        const a = ateos.datetime().hours(12).minutes(0).seconds(0);

        assert.equal(ateos.datetime(a).calendar(), "dnes o 12:00", "today at the same time");
        assert.equal(ateos.datetime(a).add({
            m: 25
        }).calendar(), "dnes o 12:25", "Now plus 25 min");
        assert.equal(ateos.datetime(a).add({
            h: 1
        }).calendar(), "dnes o 13:00", "Now plus 1 hour");
        assert.equal(ateos.datetime(a).add({
            d: 1
        }).calendar(), "zajtra o 12:00", "tomorrow at the same time");
        assert.equal(ateos.datetime(a).subtract({
            h: 1
        }).calendar(), "dnes o 11:00", "Now minus 1 hour");
        assert.equal(ateos.datetime(a).subtract({
            d: 1
        }).calendar(), "včera o 12:00", "yesterday at the same time");
    });
    ...
    it("humanize duration", () => {
        assert.equal(ateos.datetime.duration(1, "minutes").humanize(), "minúta", "a minute (future)");
        assert.equal(ateos.datetime.duration(1, "minutes").humanize(true), "za minútu", "in a minute");
        assert.equal(ateos.datetime.duration(-1, "minutes").humanize(), "minúta", "a minute (past)");
        assert.equal(ateos.datetime.duration(-1, "minutes").humanize(true), "pred minútou", "a minute ago");
    });
