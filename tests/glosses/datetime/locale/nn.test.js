...
describe("datetime", "locale", "nn", () => {
    ...
    it("suffix", () => {
        assert.equal(ateos.datetime(30000).from(0), "om nokre sekund", "prefix");
        assert.equal(ateos.datetime(0).from(30000), "nokre sekund sidan", "suffix");
    });

    it("now from now", () => {
        assert.equal(ateos.datetime().fromNow(), "nokre sekund sidan", "now from now should display as in the past");
    });
    ...
    it("calendar day", () => {
        ...
        assert.equal(ateos.datetime(a).calendar(), "I dag klokka 12:00", "today at the same time");
