describe("datetime", "invalid", () => {
    before(() => {
        ateos.datetime.locale("en");
    });

    it("invalid", () => {
        const m = ateos.datetime.invalid();
    ...
    it("invalid with existing flag", () => {
        const m = ateos.datetime.invalid({ invalidMonth: "whatchamacallit" });
        assert.equal(m.isValid(), false);
    ...
    it("invalid with custom flag", () => {
        const m = ateos.datetime.invalid({ tooBusyWith: "reiculating splines" });
        assert.equal(m.isValid(), false);
        assert.equal(m.parsingFlags().userInvalidated, false);
    ...
    it("invalid operations", () => {
        const invalids = [
            ateos.datetime.invalid(),
            ateos.datetime("xyz", "l"),
            ateos.datetime("2015-01-35", "YYYY-MM-DD"),
            ateos.datetime("2015-01-25 a", "YYYY-MM-DD", true)
        ];

        const valid = ateos.datetime();
        ...
        for (let i = 0; i < invalids.length; ++i) {
            ...
            assert.ok(!ateos.datetime.min(invalid, valid).isValid());
            assert.ok(!ateos.datetime.min(valid, invalid).isValid());
            assert.ok(!ateos.datetime.max(invalid, valid).isValid());
            assert.ok(!ateos.datetime.max(valid, invalid).isValid());
            ...
            assert.equal(ateos.datetime.duration({ from: invalid, to: valid }).asMilliseconds(), 0);
            assert.equal(ateos.datetime.duration({ from: valid, to: invalid }).asMilliseconds(), 0);
            assert.equal(ateos.datetime.duration({ from: invalid, to: invalid }).asMilliseconds(), 0);
        }
    });
});
