describe("datetime", "add and subtract", () => {
    ...
    it("add short reverse args", () => {
        ...
        const b = ateos.datetime([2010, 0, 31]).add({ M: 1 });
        const c = ateos.datetime([2010, 1, 28]).subtract({ M: 1 });
        const d = ateos.datetime([2010, 1, 28]).subtract({ Q: 1 });
    ...
    it("add across DST", () => {
        ...
        const a = ateos.datetime(new Date(2011, 2, 12, 5, 0, 0));
        const b = ateos.datetime(new Date(2011, 2, 12, 5, 0, 0));
        const c = ateos.datetime(new Date(2011, 2, 12, 5, 0, 0));
        const d = ateos.datetime(new Date(2011, 2, 12, 5, 0, 0));
        const e = ateos.datetime(new Date(2011, 2, 12, 5, 0, 0));
        a.add(1, "days");
        b.add(24, "hours");
        c.add(1, "months");
        e.add(1, "quarter");
    ...
    it("add decimal values of days and months", () => {
        assert.equal(ateos.datetime([2016, 3, 3]).add(1.5, "days").date(), 5, "adding 1.5 days is rounded to adding 2 day");
        assert.equal(ateos.datetime([2016, 3, 3]).add(-1.5, "days").date(), 1, "adding -1.5 days is rounded to adding -2 day");
        assert.equal(ateos.datetime([2016, 3, 1]).add(-1.5, "days").date(), 30, "adding -1.5 days on first of month wraps around");
        assert.equal(ateos.datetime([2016, 3, 3]).add(1.5, "months").month(), 5, "adding 1.5 months adds 2 months");
        assert.equal(ateos.datetime([2016, 3, 3]).add(-1.5, "months").month(), 1, "adding -1.5 months adds -2 months");
        assert.equal(ateos.datetime([2016, 0, 3]).add(-1.5, "months").month(), 10, "adding -1.5 months at start of year wraps back");
        assert.equal(ateos.datetime([2016, 3, 3]).subtract(1.5, "days").date(), 1, "subtract 1.5 days is rounded to subtract 2 day");
        assert.equal(ateos.datetime([2016, 3, 2]).subtract(1.5, "days").date(), 31, "subtract 1.5 days subtracts 2 days");
        assert.equal(ateos.datetime([2016, 1, 1]).subtract(1.1, "days").date(), 31, "subtract 1.1 days wraps to previous month");
        assert.equal(ateos.datetime([2016, 3, 3]).subtract(-1.5, "days").date(), 5, "subtract -1.5 days is rounded to subtract -2 day");
        assert.equal(ateos.datetime([2016, 3, 30]).subtract(-1.5, "days").date(), 2, "subtract -1.5 days on last of month wraps around");
        assert.equal(ateos.datetime([2016, 3, 3]).subtract(1.5, "months").month(), 1, "subtract 1.5 months subtract 2 months");
        assert.equal(ateos.datetime([2016, 3, 3]).subtract(-1.5, "months").month(), 5, "subtract -1.5 months subtract -2 month");
        assert.equal(ateos.datetime([2016, 11, 31]).subtract(-1.5, "months").month(), 1, "subtract -1.5 months at end of year wraps back");
        assert.equal(ateos.datetime([2016, 0, 1]).add(1.5, "years").format("YYYY-MM-DD"), "2017-07-01", "add 1.5 years adds 1 year six months");
        assert.equal(ateos.datetime([2016, 0, 1]).add(1.6, "years").format("YYYY-MM-DD"), "2017-08-01", "add 1.6 years becomes 1.6*12 = 19.2, round, 19 months");
        assert.equal(ateos.datetime([2016, 0, 1]).add(1.1, "quarters").format("YYYY-MM-DD"), "2016-04-01", "add 1.1 quarters 1.1*3=3.3, round, 3 months");
    });
});
