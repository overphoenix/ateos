...
describe("error", () => {
    describe("cleanStack", () => {
        it("default", () => {
            ...
            assert.equal(cleanStack(stack), pre);
        });
        ...
        it("works on Windows", () => {
            ...
            at Pipe.channel.onread (internal\\child_process.js:440:11)\n
            at process.emit (events.js:172:7)`;
            assert.equal(cleanStack(stack), expected);
        });
    ...
    it("AggregateException", () => {
        ...
        console.log(err);
        assert.match(err.message, /Error: foo\n {8}at /);
        assert.match(err.message, /Error: bar\n {8}at /);
