...
const srcPath = (...args) => ateos.getPath("lib", "glosses", "datastores", "backends", "pubsub", ...args);
...
describe("datastore", "backend", "PubsubDatastore", function () {
    ...
    // create PubsubDatastore instances
    before((done) => {
    ...
    it("should subscribe the topic, but receive error as no entry is stored locally", (done) => {
    ...
    it("should put correctly to daemon A and daemon B should not receive it without subscribing", (done) => {
        const dsPubsubA = new PubsubDatastore(pubsubA, datastoreA, peerIdA, smoothValidator);
    ...
    it("should validate if record content is the same", (done) => {
    ...
    it("should put correctly to daemon A and daemon B should receive it as it tried to get it first and subscribed it", (done) => {
        const dsPubsubA = new PubsubDatastore(pubsubA, datastoreA, peerIdA, smoothValidator);
        const dsPubsubB = new PubsubDatastore(pubsubB, datastoreB, peerIdB, smoothValidator);
        const subsTopic = keyToTopic(`/${keyRef}`);
        let receivedMessage = false;
    ...
    it("should fail to create the PubsubDatastore if no validator is provided", (done) => {
        let dsPubsubB;
        try {
            dsPubsubB = new PubsubDatastore(pubsubB, datastoreB, peerIdB); // no validator
        } catch (err) {
            expect(err).to.exist();
        }
    ...
    it("should fail to create the PubsubDatastore if no validate function is provided", (done) => {
        const customValidator = {
            validate: undefined,
            select: (receivedRecod, currentRecord, callback) => {
                callback(null, 0);
    ...
    it("should fail to create the PubsubDatastore if no select function is provided", (done) => {
        const customValidator = {
            validate: (data, peerId, callback) => {
                callback(null, true);
            },
    ...
    it("should fail if it fails to validate the record", (done) => {
        const customValidator = {
            validate: (data, peerId, callback) => {
    ...
    it("should get the second record if the selector selects it as the newest one", (done) => {
        const customValidator = {
            validate: (data, peerId, callback) => {
                callback(null, true);
            },
    ...
    it("should get the new record if the selector selects it as the newest one", (done) => {
        const customValidator = {
            validate: (data, peerId, callback) => {
                callback(null, true);
            },
    ...
    it("should subscribe the topic and after a message being received, discard it using the subscriptionKeyFn", (done) => {
    ...
    it("should subscribe the topic and after a message being received, change its key using subscriptionKeyFn", (done) => {
        const subscriptionKeyFn = (topic, callback) => {
    ...
    it("should subscribe a topic only once", (done) => {
        const dsPubsubA = new PubsubDatastore(pubsubA, datastoreA, peerIdA, smoothValidator);
    ...
    it("should handle a unexpected error properly when getting from the datastore", (done) => {
        const dsPubsubA = new PubsubDatastore(pubsubA, datastoreA, peerIdA, smoothValidator);
