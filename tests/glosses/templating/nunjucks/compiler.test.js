...
describe("templating", "nunjucks", "compiler", () => {
    const { std: { fs, path }, templating: { nunjucks: { Template } } } = ateos;
    ...
    it("should escape newlines", (done) => {
        equal("foo\\nbar", "foo\\nbar");
        finish(done);
    });

    it("should compile references", (done) => {
        equal("{{ foo.bar }}",
            { foo: { bar: "baz" } },
            "baz");
    ...
    it("should not treat falsy values the same as undefined", (done) => {
        equal("{{ foo }}", { foo: 0 }, "0");
        equal("{{ foo }}", { foo: false }, "false");
        finish(done);
    });

    it("should display none as empty string", (done) => {
        equal("{{ none }}", "");
        finish(done);
    });

    it("should compile none as falsy", (done) => {
        equal("{% if not none %}yes{% endif %}", "yes");
        finish(done);
    });

    it("should compile none as null, not undefined", (done) => {
        equal('{{ none|default("d", false) }}', "");
        finish(done);
    });

    it("should compile function calls", (done) => {
        equal('{{ foo("msg") }}',
            {
                foo(str) {
                    return `${str}hi`;
    ...
    it("should compile function calls with correct scope", (done) => {
        equal("{{ foo.bar() }}", {
            foo: {
                bar() {
                    return this.baz;
    ...
    it("should compile the ternary operator", (done) => {
    ...
    it("should compile inline conditionals", (done) => {
    ...
    function runLoopTests(block, end) {
        ...
        expect(res).to.be.equal("showing test\nshowing 1\nshowing 2\nshowing 3\n");
    }

    it("should compile for blocks", (done) => {
    ...
    it("should compile asyncEach", (done) => {
        runLoopTests("asyncEach", "endeach");
        finish(done);
    });

    it("should compile asyncAll", (done) => {
        runLoopTests("asyncAll", "endall");
        finish(done);
    });

    it("should compile async control", (done) => {
        if (fs) {
            const opts = {
                asyncFilters: {
                    getContents(tmpl, cb) {
                    ...
                    getContentsArr(arr, cb) {
                        fs.readFile(arr[0], (err, res) => {
                            cb(err, [res]);
                        });
                    }
                }
            ...
            render("{{ tmpl | getContents }}",
                ...
                opts,
                (err, res) => {
                    expect(res).to.be.equal("somecontenthere");
                });
            ...
            render("{% block content %}hello{% endblock %} {{ tmpl | getContents }}",
                ...
                opts,
                (err, res) => {
                    expect(res).to.be.equal("hello somecontenthere");
                });
            ...
            render('{% asyncAll i in [0, 1, 2, 3, 4] %}-{{ i }}:{% include "async.njk" %}-{% endall %}',
                ...
                opts,
                (err, res) => {
                    expect(res).to.be.equal("-0:somecontenthere\n-" +
                        "-1:somecontenthere\n-" +
                        "-2:somecontenthere\n-" +
                        "-3:somecontenthere\n-" +
                        "-4:somecontenthere\n-");
    ...
    it("should compile operators", (done) => {
        equal("{{ 3 + 4 - 5 * 6 / 10 }}", "4");
        equal("{{ 4**5 }}", "1024");
        equal("{{ 9//5 }}", "1");
        equal("{{ 9%5 }}", "4");
    ...
    it("should compile string concatenations with tilde", (done) => {
    ...
    it("should compile macros with only keyword args", (done) => {
    ...
    it("should compile macros with keyword args overriding defaults", (done) => {
        equal("{% macro foo(x, y, z=5) %}{{ x }}{{ y }}{{ z }}" +
            "{% endmacro %}" +
            "{{ foo(x=1, y=2, z=3) }}",
        "123");
    ...
    it("should compile macros with out-of-order keyword args", (done) => {
    ...
    it("should compile macros with multiple overridden arg defaults", (done) => {
        equal("{% macro foo(x, y=2, z=5) %}{{ x }}{{ y }}{{ z }}" +
            "{% endmacro %}" +
            "{{ foo(1, 10, 20) }}",
        "11020");
    ...
    it("should compile macro calls inside blocks", (done) => {
    ...
    it("should compile macros defined in one block and called in another", (done) => {
        equal("{% block bar %}" +
            "{% macro foo(x, y=2, z=5) %}{{ x }}{{ y }}{{ z }}" +
            "{% endmacro %}" +
            "{% endblock %}" +
    ...
    it("should compile macros that set vars", (done) => {
    ...
    it("should not leak variables set in macro to calling scope", (done) => {
        equal("{% macro setFoo() %}" +
            '{% set x = "foo" %}' +
            "{{ x }}" +
            "{% endmacro %}" +
    ...
    it("should not leak variables set in nested scope within macro out to calling scope", (done) => {
        equal("{% macro setFoo() %}" +
    ...
    it("should compile macros without leaking set to calling scope", (done) => {
    ...
    it("should compile macros that cannot see variables in caller scope", (done) => {
    ...
    it("should compile call blocks", (done) => {
        equal("{% macro wrap(el) %}" +
            "<{{ el }}>{{ caller() }}</{{ el }}>" +
            "{% endmacro %}" +
    ...
    it("should compile call blocks with args", (done) => {
    ...
    it("should import template objects", (done) => {
    ...
    it("should import templates with context", (done) => {
        equal('{% set bar = "BAR" %}' +
    ...
    it("should import templates without context", (done) => {
        equal('{% set bar = "BAR" %}' +
    ...
    it("should default to importing without context", (done) => {
        equal('{% set bar = "BAR" %}' +
    ...
    it("should inherit templates", (done) => {
    ...
    it("should inherit template objects", (done) => {
    ...
    it("should error if same block is defined multiple times", (done) => {
        const func = function () {
        ...
        expect(func).to.throw(/Block "test" defined more than once./);
    ...
    it("should render blocks in their own scope", (done) => {
    ...
    it("should include templates", (done) => {
        equal('hello world {% include "include.njk" %}',
            "hello world FooInclude ");
        finish(done);
    });

    it("should include 130 templates without call stack size exceed", (done) => {
        equal('{% include "includeMany.njk" %}',
            new Array(131).join("FooInclude \n"));
        finish(done);
    });

    it("should include templates with context", (done) => {
        equal('hello world {% include "include.njk" %}',
            { name: "james" },
    ...
    it("should include templates that can see including scope, but not write to it", (done) => {
        equal('{% set var = 1 %}{% include "include-set.njk" %}{{ var }}', "12\n1");
        finish(done);
    });

    it("should include templates dynamically", (done) => {
    ...
    it("should include templates dynamically based on a set var", (done) => {
    ...
    it("should include templates dynamically based on an object attr", (done) => {
    ...
    it("should include template objects", (done) => {
    ...
    it("should throw an error when including a file that does not exist", (done) => {
        render(
            '{% include "missing.njk" %}',
            {},
            { noThrow: true },
    ...
    it("should fail silently on missing templates if requested", (done) => {
     ...
     */
    it('should have access to "loop" inside an include', (done) => {
    ...
    it("should maintain nested scopes", (done) => {
        equal("{% for i in [1,2] %}" +
    ...
    it("should make includes inherit scope", (done) => {
    ...
    it("should compile a set block", (done) => {
    ...
    it("should compile set with frame references", (done) => {
    ...
    it("should compile set assignments of the same variable", (done) => {
        equal('{% set x = "hello" %}' +
    ...
    it("should allow custom tag compilation", (done) => {
        ...
        render("{% test %}123456789{% endtest %}", null, opts, (err, res) => {
            expect(res).to.be.equal("987654321");
        });

        finish(done);
    });

    it("should allow custom tag compilation without content", (done) => {
        function testExtension() {
            // jshint validthis: true
            this.tags = ["test"];
        ...
        const opts = { extensions: { testExtension: new testExtension() } };
        render('{% test "123456" %}', null, opts, (err, res) => {
            expect(res).to.be.equal("654321");
        });

        finish(done);
    });

    it("should allow complicated custom tag compilation", (done) => {
        function testExtension() {
            // jshint validthis: true
            this.tags = ["test"];
        ...
        render("{% test %}abcdefg{% endtest %}", null, opts, (err, res) => {
            expect(res).to.be.equal("a,b,c,d,e,f,g");
        });
        ...
        render("{% test %}abcdefg{% intermediate %}second half{% endtest %}",
            ...
            opts,
            (err, res) => {
                expect(res).to.be.equal("a,b,c,d,e,f,gflah dnoces");
            });
    ...
    it("should allow custom tag with args compilation", (done) => {
        ...
        render("{% test %}foobar{% endtest %}", null, opts, (err, res) => {
            expect(res).to.be.equal("raboof");
        });

        render('{% test("biz") %}foobar{% endtest %}', null, opts, (err, res) => {
            expect(res).to.be.equal("bizraboof");
        });

        render('{% test("biz", cutoff=5) %}foobar{% endtest %}', null, opts, (err, res) => {
            expect(res).to.be.equal("bizra");
        });

        finish(done);
    });

    it("should autoescape by default", (done) => {
        equal("{{ foo }}", { foo: '"\'<>&' }, "&quot;&#39;&lt;&gt;&amp;");
        finish(done);
    });

    it("should autoescape if autoescape is on", (done) => {
        render("{{ foo }}", { foo: '"\'<>&' }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("&quot;&#39;&lt;&gt;&amp;");
        });

        render("{{ foo|reverse }}", { foo: '"\'<>&' }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("&amp;&gt;&lt;&#39;&quot;");
        });

        render("{{ foo|reverse|safe }}", { foo: '"\'<>&' }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal('&><\'"');
        });

        render("{{ foo }}", { foo: null }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("");
        });

        render("{{ foo }}", { foo: ["<p>foo</p>"] }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("&lt;p&gt;foo&lt;/p&gt;");
        });
        ...
        }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("&lt;p&gt;foo&lt;/p&gt;");
        });

        render("{{ foo | safe }}", { foo: null }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("");
        });

        render("{{ foo | safe }}", { foo: "<p>foo</p>" }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal("<p>foo</p>");
        });
    ...
    it("should not autoescape safe strings", (done) => {
        render("{{ foo|safe }}", { foo: '"\'<>&' }, { autoescape: true }, (err, res) => {
            expect(res).to.be.equal('"\'<>&');
        });
    ...
    it("should not autoescape macros", (done) => {
        render(
    ...
    it("should not autoescape super()", (done) => {
        render(
            '{% extends "base3.njk" %}' +
            "{% block block1 %}{{ super() }}{% endblock %}",
            null,
    ...
    it("should not autoescape when extension set false", (done) => {
        function testExtension() {
            // jshint validthis: true
            this.tags = ["test"];
    ...
    it("should pass context as this to filters", (done) => {
        render(
            "{{ foo | hallo }}",
            { foo: 1, bar: 2 },
            {
    ...
    describe("the filter tag", () => {
        ...
        it("should apply the title filter to the body", (done) => {
            equal("{% filter title %}may the force be with you{% endfilter %}",
                "May The Force Be With You");
            finish(done);
        });

        it("should apply the replace filter to the body", (done) => {
        ...
        it("should work with variables in the body", (done) => {
    ...
    it("should throw an error when including a file that calls an undefined macro", (done) => {
        render(
    ...
    it("should throw an error when including a file that calls an undefined macro even inside {% if %} tag", (done) => {
        render(
    ...
    it("should throw an error when including a file that imports macro that calls an undefined macro", (done) => {
        render(
    ...
    it("should control whitespaces correctly", (done) => {
        equal(
    ...
    it("should control expression whitespaces correctly", (done) => {
        equal(
    ...
    it("should get right value when macro parameter conflict with global macro name", (done) => {
        render(
            ...
            "{{macro2(\"this should be outputted\") }}", {}, {}, (err, res) => {
    ...
    it("should get right value when macro include macro", (done) => {
        render(
            `${String(`${String("{# macro1 and macro2 definition #}" +
                "{% macro macro1() %} foo" +
                "{% endmacro %}")
