...
describe("templating", "nunjucks", "filter", () => {
    const { is, templating: { nunjucks: { runtime: r } } } = ateos;
    ...
    it("escape", (done) => {
        const res = render('{{ "<html>" | escape }}', {}, { autoescape: false });
        expect(res).to.be.equal("&lt;html&gt;");
        finish(done);
    });

    it("escape skip safe", (done) => {
        const res = render('{{ "<html>" | safe | escape }}', {}, { autoescape: false });
        expect(res).to.be.equal("<html>");
        finish(done);
    });

    it("should not double escape strings", (done) => {
        const res = render('{{ "<html>" | escape | escape }}', {}, { autoescape: false });
        expect(res).to.be.equal("&lt;html&gt;");
        finish(done);
    });

    it("should not double escape with autoescape on", (done) => {
        const res = render('{% set val = "<html>" | escape %}{{ val }}', {}, { autoescape: true });
        expect(res).to.be.equal("&lt;html&gt;");
        finish(done);
    });

    it("should work with non-string values", (done) => {
        const res1 = render("{{ foo | escape }}", { foo: ["<html>"] }, { autoescape: false });
        expect(res1).to.be.equal("&lt;html&gt;");

        const res2 = render("{{ foo | escape }}", {
            foo: {
                toString() {
                    return "<html>";
                }
        ...
        expect(res2).to.be.equal("&lt;html&gt;");

        const res3 = render("{{ foo | escape }}", { foo: null }, { autoescape: false });
        expect(res3).to.be.equal("");

        finish(done);
    });

    it("should not escape safe strings with autoescape on", (done) => {
        const res1 = render('{{ "<html>" | safe | escape }}', {}, { autoescape: true });
        expect(res1).to.be.equal("<html>");

        const res2 = render('{% set val = "<html>" | safe | e %}{{ val }}', {}, { autoescape: true });
        expect(res2).to.be.equal("<html>");
        finish(done);
    });

    it("should keep strings escaped after they have been escaped", (done) => {
        const res = render('{% set val = "<html>" | e | safe %}{{ val }}', {}, { autoescape: false });
        expect(res).to.be.equal("&lt;html&gt;");
        finish(done);
    });
    ...
    it("rejectattr", (done) => {
        ...
        equal('{{ foods | rejectattr("tasty") | length }}', { foods }, "1");
        finish(done);
    });

    it("selectattr", (done) => {
        const foods = [{ tasty: true }, { tasty: false }, { tasty: true }];
        equal('{{ foods | selectattr("tasty") | length }}', { foods }, "2");
        finish(done);
    });
