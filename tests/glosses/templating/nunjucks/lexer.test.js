describe("templating", "nunjucks", "lexer", () => {
    const { is, templating: { nunjucks: { lexer } } } = ateos;
    ...
    const hasTokens = (tokens, ...types) => {
        return _hasTokens(false, tokens, types);
    };

    const hasTokensWithWS = (tokens, ...types) => {
        return _hasTokens(true, tokens, types);
    };

    it("should parse template data", () => {
        let tok = lexer.lex("3").nextToken();
        expect(tok.type).to.be.equal(lexer.TOKEN_DATA);
    ...
    it("should keep track of whitespace", () => {
        const tokens = lexer.lex("data {% 1 2\n   3  %} data");
    ...
    it("should trim blocks", () => {
        const tokens = lexer.lex("  {% if true %}\n    foo\n  {% endif %}\n", { trimBlocks: true });
        hasTokens(tokens,
            [lexer.TOKEN_DATA, "  "],
            lexer.TOKEN_BLOCK_START,
            lexer.TOKEN_SYMBOL,
    ...
    it("should not trim CR after blocks", () => {
        const tokens = lexer.lex("  {% if true %}\r    foo\r\n  {% endif %}\r", { trimBlocks: true });
        hasTokens(tokens,
            [lexer.TOKEN_DATA, "  "],
            lexer.TOKEN_BLOCK_START,
            lexer.TOKEN_SYMBOL,
    ...
    it("should lstrip and not collapse whitespace between blocks", () => {
        const tokens = lexer.lex("   {% t %} {% t %}", { lstripBlocks: true });
        hasTokens(tokens,
            lexer.TOKEN_BLOCK_START,
            lexer.TOKEN_SYMBOL,
            lexer.TOKEN_BLOCK_END,
    ...
    it("should parse variable start and end", () => {
        const tokens = lexer.lex("data {{ foo }} bar bizzle");
        hasTokens(tokens,
            lexer.TOKEN_DATA,
    ...
    it("should treat the non-breaking space as valid whitespace", () => {
        ...
        tok = tokens.nextToken();
        tok = tokens.nextToken();
        expect(tok.type).to.be.equal(lexer.TOKEN_SYMBOL);
    ...
    it("should parse basic types", () => {
        const tokens = lexer.lex('{{ 3 4.5 true false none foo "hello" \'boo\' r/regex/ }}');
        hasTokens(tokens,
            lexer.TOKEN_VARIABLE_START,
            lexer.TOKEN_INT,
            lexer.TOKEN_FLOAT,
    ...
    it("should parse function calls", () => {
        const tokens = lexer.lex("{{ foo(bar) }}");
        hasTokens(tokens,
            lexer.TOKEN_VARIABLE_START,
    ...
    it("should parse dicts", () => {
        const tokens = lexer.lex('{{ {one:1, "two":2} }}');
        hasTokens(tokens,
            lexer.TOKEN_VARIABLE_START,
    ...
    it("should parse blocks without whitespace", () => {
        const tokens = lexer.lex("data{{hello}}{%if%}data");
        hasTokens(tokens,
            lexer.TOKEN_DATA,
            lexer.TOKEN_VARIABLE_START,
    ...
    it("should parse comments", () => {
        const tokens = lexer.lex("data data {# comment #} data");
        hasTokens(tokens,
            lexer.TOKEN_DATA,
            lexer.TOKEN_COMMENT,
            lexer.TOKEN_DATA);
    ...
    it("should allow changing the variable start and end", () => {
        const tokens = lexer.lex("data {= var =}", { tags: { variableStart: "{=", variableEnd: "=}" } });
        hasTokens(tokens,
            lexer.TOKEN_DATA,
            lexer.TOKEN_VARIABLE_START,
            lexer.TOKEN_SYMBOL,
    ...
    it("should allow changing the block start and end", () => {
        const tokens = lexer.lex("{= =}", { tags: { blockStart: "{=", blockEnd: "=}" } });
        hasTokens(tokens,
            lexer.TOKEN_BLOCK_START,
            lexer.TOKEN_BLOCK_END);
    }),
    ...
    it("should allow changing the comment start and end", () => {
        const tokens = lexer.lex("<!-- A comment! -->", { tags: { commentStart: "<!--", commentEnd: "-->" } });
        hasTokens(tokens,
            lexer.TOKEN_COMMENT);
    }),
    ...
    it("should have individual lexer tag settings for each environment", () => {
        let tokens = lexer.lex("{=", { tags: { variableStart: "{=" } });
    ...
    it("should parse regular expressions", () => {
        let tokens = lexer.lex("{{ r/basic regex [a-z]/ }}");
        hasTokens(tokens,
            lexer.TOKEN_VARIABLE_START,
            lexer.TOKEN_REGEX,
            lexer.TOKEN_VARIABLE_END);
