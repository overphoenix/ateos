import { concat } from "./settings";
import { clone, merge, flatten } from "../../utils";
...
const __ = ateos.lazify({
  ...
  schemas: () => {
    ...
    return model.object({
      ...
      allowUnknown: model.boolean(),
      skipFunctions: model.boolean(),
      stripUnknown: [model.boolean(), model.object({ arrays: model.boolean(), objects: model.boolean() }).or("arrays", "objects")],
      language: model.object(),
      presence: model.string().only("required", "optional", "forbidden", "ignore"),
      raw: model.boolean(),
      context: model.object(),
      strip: model.boolean(),
      noDefaults: model.boolean(),
      escapeHtml: model.boolean()
    }).strict();
  }
}, exports, require);
...
export default class Any {
  ...
  concat(schema) {
    assert(schema instanceof Any, "Invalid schema object");
    assert(this._type === "any" || schema._type === "any" || schema._type === this._type, `Cannot merge type ${this._type} with another type: ${schema._type}`);
    ...
    obj._settings = obj._settings ? concat(obj._settings, schema._settings) : schema._settings;
    obj._valids.merge(schema._valids, schema._invalids);
    obj._invalids.merge(schema._invalids, schema._valids);
    obj._tests = obj._tests.concat(schema._tests);
    obj._refs = obj._refs.concat(schema._refs);
    merge(obj._flags, schema._flags);
  ...
  options(options) {
    assert(!options.context, "Cannot override context");
    this.checkOptions(options);

    const obj = this.clone();
    obj._settings = concat(obj._settings, options);
    return obj;
  }
  ...
  example(...args) {
    assert(args.length === 1, "Missing example");
    const value = args[0];
  ...
  _validate(value, state, options, reference) {
    ...
    if (this._settings) {
      options = concat(options, this._settings);
    }
  ...
  describe() {
    ...
    if (this._settings) {
      description.options = clone(this._settings);
    }
...
Any.prototype.isImmutable = true; // Prevents from deep cloning schema objects

// Aliases

Any.prototype.only = Any.prototype.equal = Any.prototype.valid;
