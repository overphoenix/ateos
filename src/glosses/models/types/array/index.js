import Any from "../any";
import { flatten, reach, deepEqual } from "../../utils";
...
internals.Array = class extends Any {
  ...
  _checkItems(items, wasArray, state, options) {
    ...
    for (let i = 0; i < il; ++i) {
      ...
      const stripUnknown = options.stripUnknown
        ? (options.stripUnknown === true ? true : Boolean(options.stripUnknown.arrays))
        : false;
  ...
  min(limit) {
    ...
    return this._test("min", limit, function (value, state, options) {
      ...
      if (isRef_) {
        ...
        if (!(is.safeInteger(compareTo) && compareTo >= 0)) {
  ...
  _fillOrderedErrors(errors, ordereds, state, options) {
    ...
    for (let i = 0; i < ordereds.length; ++i) {
      const presence = reach(ordereds[i], "_flags.presence");
