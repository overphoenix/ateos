...
const deferredHandlers: {
	...
	MISSING_EXPORT: {
		fn: warnings => {
			...
			warnings.forEach(warning => {
				...
				stderr(chalk.gray(warning.frame as string));
			});
		},
		priority: 1
	},
	...
	NAMESPACE_CONFLICT: {
		fn: warnings => {
			...
			warnings.forEach(warning => {
				stderr(
					`${chalk.bold(relativeId(warning.reexporter as string))} re-exports '${
						warning.name
	...
	MISSING_GLOBAL_NAME: {
		fn: warnings => {
			...
			warnings.forEach(warning => {
				stderr(`${chalk.bold(warning.source as string)} (guessing '${warning.guess}')`);
			});
		},
		priority: 1
	},
...
function showTruncatedWarnings(warnings: RollupWarning[]) {
	...
	sliced.forEach(({ key: id, items }) => {
		...
		stderr(chalk.gray(items[0].frame as string));
