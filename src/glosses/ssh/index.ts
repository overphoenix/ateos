...
export class HappySSH {
  ...
  public async connect(givenConfig: SSHConfig): Promise<this> {
    invariant(
      givenConfig != null && typeof givenConfig === 'object',
      'config must be a valid object',
    );

    const config: SSHConfig = { ...givenConfig };
    ...
    if (config.privateKey != null || config.privateKeyPath != null) {
      if (config.privateKey != null) {
        ...
        invariant(
          ...
          'config.privateKeyPath must be a valid string',
        );
        invariant(
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.exec(
        ...
        (err, channel) => {
          ...
          channel.on('exit', (code_: any, signal_: any) => {
            code = code_ ?? null;
            signal = signal_ ?? null;
          });
