...
export const delay = (ms, value, { unref = false } = {}) => {
  return new Promise((resolve) => {
    const timer = ateos.setTimeout(resolve, ms, value);
    if (unref) {
      timer.unref();
    }
  });
...
export const timeout = (promise, ms) => {
  if (!is.promise(promise)) {
    throw new ateos.error.InvalidArgumentException("The first argument must be a promise");
  }
  return new Promise((resolve, reject) => {
    const timestamp = new Date();
    const timer = ateos.setTimeout(() => {
      reject(new ateos.error.TimeoutException(`Timeout of ${ms}ms exceeded`));
    }, ms);
    promise.then((x) => {
      ateos.clearTimeout(timer);
      ...
      ateos.clearTimeout(timer);
      if (new Date() - timestamp >= ms) {
        const err = new ateos.error.TimeoutException(`Timeout of ${ms}ms exceeded`);
        err.original = y;
        reject(err);
      } else {
        reject(y);
...
export const callbackify = (fn) => {
  if (!is.function(fn)) {
    throw new ateos.error.InvalidArgumentException("The first argument must be a function");
  }
 ...
 */
export const promisifyAll = (source, { suffix = "Async", filter = ateos.truly, context } = {}) => {
...
const _finally = (promise, onFinally) => {
  onFinally = onFinally || ateos.noop;
