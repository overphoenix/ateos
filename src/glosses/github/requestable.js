...
export default class Requestable {
  ...
  _request(method, path, data, cb, raw) {
    ...
    const requestPromise = ateos.http.client.request(config).catch(callbackErrorOrThrow(cb, path));
    ...
    return this._request("GET", path, options)
      .then((response) => {
        ...
        if (response.data instanceof Array) {
          ...
          const message = `cannot figure out how to append ${response.data} to the result set`;
