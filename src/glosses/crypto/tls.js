 ...
 * @return the pseudo random bytes in a byte buffer.
 */
const prf_sha256 = function (secret, label, seed, length) {
 ...
 */
const hmac_sha1 = function (key, seqNum, record) {
  /**
 ...
 */
const deflate = function (c, record, s) {
  let rval = false;
 ...
 * @return true on success, false on failure.
 */
const inflate = function (c, record, s) {
  let rval = false;
 ...
 */
const readVector = function (b, lenBytes) {
  let len = 0;
  switch (lenBytes) {
    case 1:
      len = b.getByte();
 ...
 */
const writeVector = function (b, lenBytes, v) {
...
tls.handleServerHello = function (c, record, length) {
  const msg = tls.parseHelloMessage(c, record, length);
  if (c.fail) {
    return;
  }
...
const __ = tls.handleUnexpected;
const R0 = tls.handleChangeCipherSpec;
const R1 = tls.handleAlert;
const R2 = tls.handleHandshake;
const R3 = tls.handleApplicationData;
const R4 = tls.handleHeartbeat;
const ctTable = [];
...
const H0 = tls.handleHelloRequest;
const H1 = tls.handleServerHello;
const H2 = tls.handleCertificate;
const H3 = tls.handleServerKeyExchange;
const H4 = tls.handleCertificateRequest;
const H5 = tls.handleServerHelloDone;
const H6 = tls.handleFinished;
var hsTable = [];
...
const H7 = tls.handleClientHello;
const H8 = tls.handleClientKeyExchange;
const H9 = tls.handleCertificateVerify;
...
tls.createConnectionState = function (c) {
  ...
  const createMode = function () {
    var mode = {
      ...
      macFunction: null,
      cipherState: null,
      cipherFunction(record) {
        return true; 
      },
      compressionState: null,
      compressFunction(record) {
        return true; 
      },
 ...
 */
const _certErrorToAlertDesc = function (error) {
  switch (error) {
    case true:
      return true;
 ...
 */
const _alertDescToCertError = function (desc) {
  switch (desc) {
    case true:
      return true;
    case tls.Alert.Description.bad_certificate:
