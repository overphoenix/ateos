...
(function (jQuery) {
  ...
  const prng_aes = {};
  const _prng_aes_output = new Array(4);
  const _prng_aes_buffer = crypto.util.createBuffer();
  prng_aes.formatKey = function (key) {
    // convert the key into 32-bit integers
    const tmp = crypto.util.createBuffer(key);
    key = new Array(4);
    key[0] = tmp.getInt32();
    key[1] = tmp.getInt32();
    key[2] = tmp.getInt32();
    ...
    return crypto.aes._expandKey(key, false);
  };
  prng_aes.formatSeed = function (seed) {
    // convert seed into 32-bit integers
    const tmp = crypto.util.createBuffer(seed);
    seed = new Array(4);
    seed[0] = tmp.getInt32();
    seed[1] = tmp.getInt32();
    seed[2] = tmp.getInt32();
  ...
  prng_aes.cipher = function (key, seed) {
    crypto.aes._updateBlock(key, seed, _prng_aes_output, false);
  ...
  prng_aes.md = crypto.md.sha256;

  /**
     * Creates a new PRNG.
     */
  function spawnPrng() {
    const ctx = crypto.prng.create(prng_aes);
