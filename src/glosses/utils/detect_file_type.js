...
const readUInt64LE = (buf, offset = 0) => {
  let n = buf[offset];
  let mul = 1;
  let i = 0;
  while (++i < 8) {
...
Object.defineProperty(detectFileType, "minimumBytes", { value: 4100 });

detectFileType.stream = (readableStream) => new Promise((resolve, reject) => {
  ...
  readableStream.once("readable", () => {
    ...
    try {
      pass.fileType = detectFileType(chunk);
    } catch (error) {
      reject(error);
    }
