...
// this code is generated by jison parser
// todo: inline it while compiling...
...
function polygonToWKTPolygon(primitive) {
  ...
  ret += "(";
  const parts = [];
  for (let i = 0; i < primitive.coordinates.length; i++) {
...
function multiPolygonToWKTMultiPolygon(primitive) {
  ...
  ret += "(";
  const inner = [];
  for (let c = 0; c < primitive.coordinates.length; c++) {
    let it = "(";
    const parts = [];
    for (let i = 0; i < primitive.coordinates[c].length; i++) {
...
function convert(primitive) {
  switch (primitive.type) {
    ...
    case "GeometryCollection":
      ...
      var parts = [];
      for (let i = 0; i < primitive.geometries.length; i++) {
