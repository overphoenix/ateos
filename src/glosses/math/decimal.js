 ...
 */
const finalise = (x, sd, rm, isTruncated) => {
    let digits;
    let i;
    let j;
    let k;
...
const divide = (function () {
    ...
    const multiplyInteger = (x, k, base) => {
        let temp;
        let carry = 0;
        let i = x.length;
...
const taylorSeries = (Ctor, n, x, y, isHyperbolic) => {
    let j;
    let t;
    let u;
 ...
 */
const checkRoundingDigits = (d, i, rm, repeating) => {
    let di;
    let k;
    let r;
    let rd;
...
const convertBase = (str, baseIn, baseOut) => {
    let j;
    const arr = [0];
    let arrL;
    let i = 0;
...
const getBase10Exponent = (digits, e) => {
    let w = digits[0];
...
const parseOther = (x, str) => {
    ...
    const xd = convertBase(str, base, BASE);
    const xe = xd.length - 1;
    ...
    if (p) {
        x = x.times(Math.abs(p) < 54 ? Math.pow(2, p) : Decimal.pow(2, p)); // eslint-disable-line no-use-before-define
    }
    external = true;
...
const naturalLogarithm = (y, sd) => {
    ...
    let x2 = finalise(x.times(x), wpr, 1);
    denominator = 3;
    ...
    for (;;) {
        ...
        if (digitsToString(t.d).slice(0, wpr) === digitsToString(sum.d).slice(0, wpr)) {
            ...
            if (e !== 0) {
                sum = sum.plus(getLn10(Ctor, wpr + 2, pr).times(String(e)));
            }
...
const naturalExponential = (x, sd) => {
    ...
    const guard = Math.log(mathpow(2, k)) / Math.LN10 * 2 + 5 | 0;
    wpr += guard;
...
// ±Infinity, NaN.
const nonFiniteToString = (x) => {
    // Unsigned.
    return String(x.s * x.s / 0);
};

const finiteToString = (x, isExp, sd) => {
    if (!x.isFinite()) {
 ...
 */
const toStringBinary = (x, baseOut, sd, rm) => {
    let base;
    let e;
    let i;
    let k;
...
const alias = (name) => (target, originalName) => {
...
class MetaDecimal {
    constructor(v) {
        ...
        const y = isDecimal.test(v) ? parseDecimal(x, v) : parseOther(x, v);

        this.s = y.s;
        this.e = y.e;
        this.d = y.d ? y.d.slice() : y.d;
    }
        ...
        for (;;) {
            ...
            if (digitsToString(t.d).slice(0, sd) === (n = digitsToString(r.d)).slice(0, sd)) {
                ...
                if (n === "9999" || !rep && n === "4999") {
                    ...
                    if (! Number(n) || ! Number(n.slice(1)) && n.charAt(0) === "5") {
    ...
    inverseCosine() {
        ...
        x = x.asin();
        const halfPi = getPi(Ctor, pr + 4, rm).times(0.5);
    ...
    inverseTangent() {
        ...
        const k = Math.min(28, wpr / LOG_BASE + 2 | 0);
        ...
        const j = Math.ceil(wpr / LOG_BASE);
        n = 1;
        const x2 = x.times(x);
        r = new Ctor(x);
        px = x;
    ...
    @alias("isInt")
    isInteger() {
        return Boolean(this.d) && mathfloor(this.e / LOG_BASE) > this.d.length - 2;
    }
     ...
     */
    isZero() {
        return Boolean(this.d) && this.d[0] === 0;
    }
        ...
        if (checkRoundingDigits(r.d, k = pr, rm)) {
            ...
            do {
                ...
                if (!inf) {
                    ...
                    if (Number(digitsToString(r.d).slice(k + 1, k + 15)) + 1 === 1e14) {
                        r = finalise(r, pr + 1, 0);
                    }
    ...
    minus(y) {
        ...
        const xe = mathfloor(x.e / LOG_BASE);
    ...
    precision(z) {
        ...
        if (z !== void 0 && z !== Boolean(z) && z !== 1 && z !== 0) {
    ...
    toFraction(maxD) {
        ...
        const r = divide(n1, d1, e, 1).minus(x).abs().cmp(divide(n0, d0, e, 1).minus(x).abs()) < 1
            ? [n1, d1]
            : [n0, d0];
    ...
    toPower(y) {
        ...
        let x = this;
        const Ctor = x.constructor;
        const yn = Number(y = new Ctor(y));

        // Either ±Infinity, NaN or ±0?
        if (!x.d || !y.d || !x.d[0] || !y.d[0]) {
            return new Ctor(mathpow(Number(x), yn));
        }
        ...
        // Truncate to the required precision plus five rounding digits.
            ...
            if (checkRoundingDigits(r.d, pr, rm)) {
                ...
                if (Number(digitsToString(r.d).slice(pr + 1, pr + 15)) + 1 === 1e14) {
                    r = finalise(r, pr + 1, 0);
                }
            }
        }
    ...
    static config(obj) {
        ...
        let p;
        let v;
        const useDefaults = obj.defaults === true;
        const ps = [
    ...
    static isDecimal(obj) {
        return obj instanceof MetaDecimal || obj && obj.name === "[object Decimal]" || false;
    }
    ...
    static random(sd) {
        ...
        if (!this.crypto) {
            ...
            // buffer
            d = ateos.crypto.random.getBytesSync(k *= 4);
            ...
            for (; i < k;) {
                ...
                if (n >= 2.14e9) {
                    ateos.crypto.random.getBytesSync(4).copy(d, i);
                } else {
...
const Decimal = MetaDecimal.clone(DEFAULTS);
