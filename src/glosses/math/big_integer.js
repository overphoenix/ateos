...
BigInteger.prototype.add = function (v) {
  ...
  const a = this.value; const b = n.value;
  if (n.isSmall) {
...
function subtract(a, b) { // assumes a and b are arrays with a >= b
  const a_l = a.length;
  const b_l = b.length;
  const r = new Array(a_l);
  let borrow = 0;
  const base = BASE;
  let i; let difference;
  for (i = 0; i < b_l; i++) {
...
function multiplyLong(a, b) {
  ...
  const base = BASE;
  let product; let carry; let i; let a_i; let b_j;
  for (i = 0; i < a_l; ++i) {
    a_i = a[i];
    for (let j = 0; j < b_l; ++j) {
      b_j = b[j];
...
BigInteger.prototype.multiply = function (v) {
  const n = parseValue(v);
  const a = this.value; let b = n.value;
...
function square(a) {
  ...
  const base = BASE;
  let product; let carry; let i; let a_i; let a_j;
  for (i = 0; i < l; i++) {
    a_i = a[i];
    carry = 0 - a_i * a_i;
    for (let j = i; j < l; j++) {
...
function divMod1(a, b) { // Left over from previous version. Performs faster than divMod2 on smaller input sizes.
  ...
  let quotientDigit; let shift; let carry; let borrow; let i; let l; let q;
...
function divMod2(a, b) { // Implementation idea shamelessly stolen from Silent Matt's library http://silentmatt.com/biginteger/
  ...
  let part = [];
  const base = BASE;
  let guess; let xlen; let highx; let highy; let check;
  while (a_l) {
    part.unshift(a[--a_l]);
    trim(part);
...
function divModSmall(value, lambda) {
  ...
  const base = BASE;
  let i; let q; let remainder; let divisor;
  remainder = 0;
...
function divModAny(self, v) {
  let value; const n = parseValue(v);
  if (supportsNativeBigInt) {
    return [new NativeBigInt(self.value / n.value), new NativeBigInt(self.value % n.value)];
  }
  const a = self.value; let b = n.value;
  let quotient;
  if (b === 0) {
...
NativeBigInt.prototype.pow = function (v) {
  const n = parseValue(v);
  const a = this.value; let b = n.value;
  const _0 = BigInt(0); const _1 = BigInt(1); const _2 = BigInt(2);
  if (b === _0) {
    return Integer[1];
  }
  if (a === _0) {
...
BigInteger.prototype.modInv = function (n) {
  let t = Integer.zero; let newT = Integer.one; let r = parseValue(n); let newR = this.abs(); let q; let lastT; let lastR;
  while (!newR.isZero()) {
    q = r.divide(newR);
    lastT = t;
    lastR = r;
...
const powers2Length = powersOfTwo.length; const highestPower2 = powersOfTwo[powers2Length - 1];
...
function bitwise(x, y, fn) {
  y = parseValue(y);
  const xSign = x.isNegative(); const ySign = y.isNegative();
  let xRem = xSign ? x.not() : x;
  let yRem = ySign ? y.not() : y;
  let xDigit = 0; let yDigit = 0;
  let xDivMod = null; let yDivMod = null;
  const result = [];
  ...
  let sum = fn(xSign ? 1 : 0, ySign ? 1 : 0) !== 0 ? Integer(-1) : Integer(0);
  for (let i = result.length - 1; i >= 0; i -= 1) {
    sum = sum.multiply(highestPower2).add(Integer(result[i]));
  }
  return sum;
}
...
const LOBMASK_I = 1 << 30; const LOBMASK_BI = (BASE & -BASE) * (BASE & -BASE) | LOBMASK_I;
...
function integerLogarithm(value, base) {
  ...
  return { p: Integer(1), e: 0 };
}

BigInteger.prototype.bitLength = function () {
  let n = this;
  if (n.compareTo(Integer(0)) < 0) {
    n = n.negate().subtract(Integer(1));
  }
  if (n.compareTo(Integer(0)) === 0) {
    return Integer(0);
  }
  return Integer(integerLogarithm(n, Integer(2)).e).add(Integer(1));
};
...
const parseBase = function (text, base, alphabet, caseSensitive) {
...
function parseBaseFromArray(digits, base, isNegative) {
  let val = Integer[0]; let pow = Integer[1]; let i;
...
function toBaseString(n, base, alphabet) {
  ...
  return (arr.isNegative ? "-" : "") + arr.value.map((x) => {
...
BigInteger.prototype.toString = function (radix, alphabet) {
  ...
  const v = this.value; let l = v.length; let str = String(v[--l]); const zeros = "0000000"; let digit;
  while (--l >= 0) {
    digit = String(v[l]);
...
function parseStringValue(v) {
  ...
  const r = []; let max = v.length; const l = LOG_BASE; let min = max - l;
  while (max > 0) {
    r.push(Number(v.slice(min, max)));
    min -= l;
    if (min < 0) {
      min = 0;
    }
