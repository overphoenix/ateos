...
export const emptyString = (str) => typeof str === "string" && /^\s*$/.test(str); //eslint-disable-line
...
export const primitive = (value) => nil(value) || number(value) || typeof value === "string" || boolean(value) || symbol(value); //eslint-disable-line
...
export const json = (value) => (typeof value === "string" && value.endsWith(".json")) || object(value); //eslint-disable-line

export const namespace = (value) => object(value) && value[ateos.asNamespace.SYMBOL] === true;
...
export const callback = (fn, names) => inArray(names || callbackNames, ateos.util.functionName(fn));
...
export const binaryPath = (x) => binaryExtensions.has(ateos.std.path.extname(x).slice(1).toLowerCase());
...
ateos.lazify({
  ...
  netronContext: () => (obj) => {
    ...
    if (!undefined_(target)) {
      isContex = object(Reflect.getMetadata(ateos.netron.meta.CONTEXT_ANNOTATION, target));
    }
    return isContex;
  }
}, exports, require);
