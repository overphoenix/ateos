module.exports = function (assertion, _) {
  const { Assertion, AssertionError } = assertion;
  const { flag } = _;
  ...
  function an(type, msg) {
    ...
    this.assert(
      type === ateos.typeOf(obj).toLowerCase()
  ...
  function include(val, msg) {
    ...
    const obj = flag(this, "object");
    const objType = ateos.typeOf(obj).toLowerCase();
    ...
    switch (objType) {
      ...
      case "map": {
        const isEql = isDeep ? _.eql : SameValueZero;
        obj.forEach((item) => {
  ...
  function assertAbove(n, msg) {
    ...
    const ssfi = flag(this, "ssfi");
    const objType = ateos.typeOf(obj).toLowerCase();
    const nType = ateos.typeOf(n).toLowerCase();
    let errorMessage;
  ...
  Assertion.addMethod("within", function (start, finish, msg) {
    ...
    const ssfi = flag(this, "ssfi");
    const objType = ateos.typeOf(obj).toLowerCase();
    const startType = ateos.typeOf(start).toLowerCase();
    const finishType = ateos.typeOf(finish).toLowerCase();
    let errorMessage;
    let shouldThrow = true;
      ...
      if (err instanceof TypeError) {
        ...
        throw new AssertionError(
          `${flagMsg}The instanceof assertion needs a constructor but ${
            ateos.typeOf(constructor)} was given.`,
          undefined,
          ssfi
        );
      }
  ...
  function assertKeys(keys) {
    const obj = flag(this, "object");
    const objType = ateos.typeOf(obj);
    const keysType = ateos.typeOf(keys);
    const ssfi = flag(this, "ssfi");
    const isDeep = flag(this, "deep");
    let str;
    let deepStr = "";
    ...
    // Has any
    if (any) {
      ok = expected.some((expectedKey) => {
        return actual.some((actualKey) => {
          if (isDeep) {
  ...
  Assertion.addMethod("members", function (subset, msg) {
    ...
    let subject; let failMsg; let failNegateMsg;
