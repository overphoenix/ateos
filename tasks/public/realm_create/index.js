...
export default class extends ateos.task.AdvancedTask {
  async main(options = {}) {
    ...
    if (!ateos.isString(options.name) || options.name.trim().length === 0) {
      throw new error.InvalidArgumentException("Invalid name of realm");
    }
    options.name = options.name.trim();

    if (!ateos.isString(options.path) || options.path.trim().length === 0) {
    ...
    if (ateos.isPlainObject(options.realm)) {
  ...
  async undo(err) {
    ...
    ateos.isString(this.cwd) && await fs.remove(this.cwd);
  }
}
