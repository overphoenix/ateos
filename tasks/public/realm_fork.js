...
export default class extends ateos.task.AdvancedTask {
  async main({ realm, path, name, tags, filter, skipNpm } = {}) {
    ...
    if (!ateos.isString(path) || path.length === 0) {
      throw new error.NotValidException(`Invalid destPath: ${ateos.inspect(path)}`);
    }

    if (!ateos.isString(name) || name.length === 0) {
      // name = realm.name;
    ...
    if (ateos.isArray(tags) && tags.length > 0) {
      tags = new Set(tags);
    } else if (ateos.isString(tags) && tags.length > 0) {
  ...
  async undo(err) {
    ...
    ateos.isString(this.destCwd) && await fs.remove(this.destCwd);
  }
}
